...
for model_mode in ${model_mode_list[@]}; do
    for fp_item in ${fp_item_list[@]}; do
          for bs_item in ${bs_item_list[@]}; do
              if [ $run_mode == "sp" ];then
                  CUDA_VISIBLE_DEVICES=0 bash $BENCHMARK_ROOT/PaddleNLP/tests/benchmark/xlnet/run_benchmark.sh ${run_mode} ${bs_item} ${fp_item} 300 ${model_mode}
              else
                  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash $BENCHMARK_ROOT/PaddleNLP/tests/benchmark/xlnet/run_benchmark.sh ${run_mode} ${bs_item} ${fp_item} 300 ${model_mode}
              fi
          done
    done
done
