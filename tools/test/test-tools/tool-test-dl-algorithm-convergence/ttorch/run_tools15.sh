...
if [ $1 = "gpt" ];then
    ...
    case ${cards} in
    1)
        res_log="nlp_gpt_sp_bs8_fp32_1" ;;
    8)
        res_log="nlp_gpt_sp_bs8_fp32_8" ;;
    *)  exit 1;
    esac
    final_res=`grep "validation loss at the end of training for test data" ${res_log} | awk '{print $(NF-1)}'`
    if [ -z ${final_res} ];then
        echo -n "trian error"
        exit 1
    fi
    python is_conv.py ${final_res} 2.52210E+04  >log.conv 2>&1
    flag=$?
    if [ $flag == 1 ];then
        echo -n "trian error"
        exit 1
