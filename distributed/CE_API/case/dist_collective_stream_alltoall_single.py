...
/***************************************************************************
  ...
  * @file dist_collective_stream_alltoall_single.py
...
def test_collective_stream_alltoall_single1_sync_calc():
    """test_collective_stream_alltoall_single1"""
    ...
    for t in types:
        ...
        paddle.distributed.stream.alltoall_single(data, output, sync_op=True, use_calc_stream=True)
        print(output)
        assert len(output) == 2
        print("test_collective_stream_alltoall_single1 %s... ok" % t)


@run_priority(level="P0")
def test_collective_stream_alltoall_single2_sync_calc():
    """test_collective_stream_alltoall_single2"""
    types = [np.float16, np.float32, np.float64, np.int32, np.int64, np.int8, np.uint8]
    for t in types:
        ...
        task = paddle.distributed.stream.alltoall_single(
            data, output, in_split_sizes, out_split_sizes, sync_op=True, use_calc_stream=True
        )
        task.wait()
        print(output)
        print("test_collective_stream_alltoall_single2 %s... ok" % t)


if __name__ == "__main__":
    test_collective_stream_alltoall_single1_sync_calc()
    test_collective_stream_alltoall_single2_sync_calc()
