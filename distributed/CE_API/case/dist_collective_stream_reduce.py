...
/***************************************************************************
  ...
  * @file dist_collective_stream_reduce.py
  * @author liujie44@baidu.com
  * @date 2023-08-09 11:10
  * @brief
  *
  **************************************************************************/
"""
...
def test_collective_stream_reduce_default():
...
@run_priority(level="P0")
def test_collective_stream_reduce_sync_calc():
    """test_collective_stream_reduce_sync_calc"""
    for t in types:
        if paddle.distributed.ParallelEnv().local_rank == 0:
            np_data = np.array([[4, 5, 6], [4, 5, 6]]).astype(t)
        else:
            np_data = np.array([[1, 2, 3], [1, 2, 3]]).astype(t)
        data = paddle.to_tensor(np_data)
        paddle.distributed.stream.reduce(data, 0, sync_op=True, use_calc_stream=True)
        out = data.numpy()
        assert len(out) == 2
