...
PS static cpu demo
export CPU_NUM=2
单机server_num:1, worker_num:1 python -m paddle.distributed.launch --server_num=1 --log_dir=pserver_log --worker_num=1 ps_static_cpu.py
单机server_num:1, worker_num:2 python -m paddle.distributed.launch --server_num=1 --log_dir=pserver_log --worker_num=2 ps_static_cpu.py
单机server_num:2, worker_num:2 python -m paddle.distributed.launch --server_num=2 --log_dir=pserver_log --worker_num=2 ps_static_cpu.py
单机server_num:2, worker_num:1 python -m paddle.distributed.launch --server_num=2 --log_dir=pserver_log --worker_num=1 ps_static_cpu.py
多机server_num:2, worker_num:2 python -m paddle.distributed.launch --servers="xx.xx.xx.xx:port1,yy.yy.yy.yy:port2" --workers="xx.xx.xx.xx,yy.yy.yy.yy" ps_static_cpu.py
"""
import math
import random
import numpy as np
...
class WideDeepModel:
    ...
    def net(self, is_train=True):
        ...
        self.loader = paddle.fluid.reader.DataLoader.from_generator(
...
class WideDeepDataset:
    ...
    def line_process(self, i):
        ...
        for idx in continuous_range_:
            dense_feature.append((float(random.randint(cont_min_[idx - 1],
                cont_max_[idx - 1])) - cont_min_[idx - 1]) / cont_diff_[idx - 1])
