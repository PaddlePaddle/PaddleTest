...
function _set_params(){
    ...
    # 以下为可选参数，请根据实际修改
    model_name_or_path=${model_name_or_path:-"facebook/llama-13b"} # 根据model_item替换
    # 根据run_mode替换更新数据
    data_parallel_degree=${data_parallel_degree:-1} # 配置默认值
    ...
    virtual_pp_degree=${virtual_pp_degree:-1}
    ...
    recompute_use_reentrant=${recompute_use_reentrant:-"false"}

    echo run_mode=${run_mode}
    sub_str="DP"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        data_parallel_degree=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="MP"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        tensor_parallel_degree=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="PP"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        pipeline_parallel_degree=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="SD"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        sharding_parallel_degree=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="stage"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        sharding=${sub_str}$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="vpp"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        virtual_pp_degree=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="mbs"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        per_device_train_batch_size=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
    fi
    sub_str="acc"
    if [[ ${run_mode} =~ ${sub_str} ]];then
        per_device_train_batch_size=$(echo $run_mode|awk -F ${sub_str} '{print $2}' | awk -F '-' '{print $1}')
