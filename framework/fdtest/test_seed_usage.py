...
def test_usage_stream():
    ...
    assert data["max_tokens"] >= usage['completion_tokens'], f"completion_tokens大于max_tokens"
    assert data["metadata"]["min_tokens"] <= usage['completion_tokens'], f"completion_tokens小于min_tokens"
    assert usage["total_tokens"] == total_tokens, f"total_tokens不等于prompt_tokens + completion_tokens"
