...
"""
test_functional_conv2d_float64
"""
...
def test_conv2d1():
    ...
    np.random.seed(obj.seed)
    x = randtool("float", 0, 1, [2, 3, 4, 4])
    weight = np.ones(shape=[1, 3, 2, 2]).astype("float64")
    bias = np.zeros(shape=[1]).astype("float64")
    stride = 1
    padding = 0
    res = np.array(
...
def test_conv2d2():
    ...
    np.random.seed(obj.seed)
    x = randtool("float", 0, 1, [2, 3, 4, 4])
    weight = np.ones(shape=[3, 3, 2, 2]).astype("float64")
    bias = np.zeros(shape=[3]).astype("float64")
    stride = 1
    padding = 0
    res = np.array(
        [
...
def test_conv2d5():
    ...
    np.random.seed(obj.seed)
    x = randtool("float", 0, 1, [2, 3, 4, 4])
    weight = np.ones(shape=[6, 1, 3, 3]).astype("float64")
    bias = np.zeros(shape=[6]).astype("float64")
    stride = 2
    padding = 0
    groups = 3
    res = np.array(
...
def test_conv2d6():
    ...
    np.random.seed(obj.seed)
    x = randtool("float", 0, 1, [2, 3, 4, 4])
    weight = np.ones(shape=[2, 3, 3, 3]).astype("float64") * 0.7
    bias = np.ones(shape=[2]).astype("float64") * -0.3
    stride = 1
    padding = 0
    res = np.array(
        [
