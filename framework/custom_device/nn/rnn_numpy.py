...
class SimpleRNNCell(LayerMixin):
    ...
    def __init__(self, input_size, hidden_size, bias=True, nonlinearity="RNN_TANH", dtype="float64"):
...
class BiRNN(LayerMixin):
    ...
    def forward(self, inputs, initial_states=None, sequence_length=None, **kwargs):
        ...
        outputs, final_states = birnn(
            self.cell_fw, self.cell_bw, inputs, initial_states, sequence_length, self.time_major
        )
        return outputs, final_states
...
class SimpleRNN(RNNMixin):
    ...
    def __init__(
        ...
        if direction in ["forward"]:
            ...
            raise ValueError(
                "direction should be forward, backward or bidirectional, " "received direction = {}".format(direction)
            )
...
class LSTM(RNNMixin):
    ...
    def __init__(
        self, input_size, hidden_size, num_layers=1, direction="forward", dropout=0.0, time_major=False, dtype="float64"
    ):
