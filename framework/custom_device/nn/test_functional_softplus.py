...
class TestFunctionalSoftplus(APIBase):
    """
    test
    """
...
obj = TestFunctionalSoftplus(paddle.nn.functional.softplus)
...
def test_softplus_base():
    ...
    obj.base(res=res, x=x, beta=beta, threshold=threshold)
...
def test_softplus():
    ...
    x_beta = beta * x
    res = np.select([x_beta <= threshold, x_beta > threshold], [np.log(1 + np.exp(x_beta)) / beta, x])
    obj.run(res=res, x=x, beta=beta, threshold=threshold)
