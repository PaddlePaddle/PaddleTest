...
class TestAdaptiveMaxPool1D(APIBase):
    """
    test
    """
...
obj = TestAdaptiveMaxPool1D(paddle.nn.AdaptiveMaxPool1D)
...
@pytest.mark.api_nn_AdaptiveAvgPool1D_parameters
...
def test_adaptive_max_pool1D3():
    ...
    """
    obj1 = TestAdaptiveMaxPool1D(paddle.nn.AdaptiveMaxPool1D)
    obj1.static = False
    np.random.seed(22)
    x = np.random.rand(1, 1, 4)
    res = np.array([[[[0.20846054, 0.48168106, 0.42053804, 0.85918200]]], [[[0, 1, 2, 3]]]])
    obj1.run(res=res, data=x, output_size=4, return_mask=True)
