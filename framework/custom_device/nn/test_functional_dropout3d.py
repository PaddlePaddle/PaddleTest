...
def test_dygraph():
    ...
    for place in places:
        for t in types:
            ...
            for i in range(1):
                out = paddle.nn.functional.dropout3d(x, p=0.5)
                out.backward()
...
def test_dygraph1():
    ...
    for place in places:
        for t in types:
            ...
            x = paddle.to_tensor(x_data, stop_gradient=False)
            for i in range(1):
                out = paddle.nn.functional.dropout3d(x, p=paddle.to_tensor(0.5))
                out.backward()
            index = np.flatnonzero(out)
            for i in index:
                assert np.allclose(x_data.flatten()[i] * 2, out.numpy().flatten()[i])
    ...
    for place in places:
        for t in types:
            ...
            with paddle.static.program_guard(main_program=main_program, startup_program=startup_program):
                input1 = paddle.static.data(name="x", shape=[2, 1, 1, 2, 3], dtype=t)
                input1.stop_gradient = False
                output = paddle.nn.functional.dropout3d(input1, p=0.5)
                g = paddle.static.gradients(output, input1)

                exe = paddle.static.Executor(place)
                exe.run(startup_program)
                x = np.arange(1, 13).reshape(2, 1, 1, 2, 3).astype(t)
