import numpy as np
import torch
import torch.nn as nn


class LayerCase(nn.Module):
    """
    case名称: embedding_zero_size_class
    """

    def __init__(self):
        super(LayerCase, self).__init__()

    def forward(self, data, weight):
        """
        forward
        """
        torch.manual_seed(33)
        np.random.seed(33)
        out = nn.functional.embedding(data, weight=weight, padding_idx=-1, sparse=False,)
        return out


def create_tensor_inputs():
    """
    PyTorch tensor
    """
    inputs = (
        torch.tensor((4 * np.random.random([0])).astype(np.int32), dtype=torch.int32, requires_grad=True), 
        torch.tensor((-1 + 2 * np.random.random([3, 2])).astype(np.float32), dtype=torch.float32, requires_grad=True),    
    )
    return inputs


def create_numpy_inputs():
    """
    numpy array
    """
    inputs = (
        4 * np.random.random([0]).astype('int32'), 
        -1 + 2 * np.random.random([3, 2]).astype('float32'),
    )
    return inputs
