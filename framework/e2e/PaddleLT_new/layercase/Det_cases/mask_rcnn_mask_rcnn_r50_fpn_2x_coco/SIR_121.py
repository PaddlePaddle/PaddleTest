...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [6, 256, 28, 28], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [6], dtype: paddle.int32, stop_gradient: True)
        var_2,    # (shape: [6, 28, 28], dtype: paddle.int32, stop_gradient: True)
        var_3,    # (shape: [6], dtype: paddle.float32, stop_gradient: True)
    ):
        ...
        var_10 = paddle.tensor.manipulation.reshape(var_9, [6, 28, 28])
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[6, 256, 28, 28], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[6, 256, 28, 28]).astype('float32'),
