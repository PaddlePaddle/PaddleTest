...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [2, 185691], dtype: paddle.float32, stop_gradient: True)
    ):
        out = paddle.tensor.search.topk(var_0, k=1, axis=0)
        var_1 = out[0]
        var_2 = out[1]
        var_3 = paddle.tensor.creation.full([1, 185691], -1, dtype='int32')
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[2, 185691], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[2, 185691]).astype('float32'),
    )
    return inputs
