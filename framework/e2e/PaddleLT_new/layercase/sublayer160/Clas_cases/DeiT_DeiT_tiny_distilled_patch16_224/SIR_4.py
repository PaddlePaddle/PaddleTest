...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[192], weight=self.parameter_5, bias=self.parameter_3, epsilon=1e-06)
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[86, 198, 192]).astype('float32'),
    )
    return inputs
