...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 192, 12, 12], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [1, 192, 12, 12], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_6 = paddle.nn.functional.conv._conv_nd(var_5, self.parameter_2, bias=self.parameter_0, stride=[1, 1], padding=[0, 0], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 192, 12, 12], dtype=paddle.float32),
        paddle.rand(shape=[1, 192, 12, 12], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 192, 12, 12]).astype('float32'),
        np.random.random(size=[1, 192, 12, 12]).astype('float32'),
    )
    return inputs
