...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1 = paddle.nn.functional.conv._conv_nd(var_0, self.parameter_3, bias=self.parameter_2, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        out = paddle.tensor.manipulation.split(var_1, num_or_sections=[18, 9], axis=1)
        var_2 = out[0]
        var_3 = out[1]
        var_4 = paddle.tensor.ops.sigmoid(var_3)
        var_5 = paddle.vision.ops.deform_conv2d(x=var_0, offset=var_2, weight=self.parameter_0, bias=self.parameter_1, stride=[1, 1], padding=[1, 1], dilation=[1, 1], deformable_groups=1, groups=1, mask=var_4)
        return var_5
