...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,  # (shape: [1, 256, 24, 36], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = var_0.shape
        ...
        var_13 = paddle.tensor.manipulation.expand(var_10, shape=[var_12, 1, 24, 36])
        var_14 = var_0.shape
        var_15 = var_14.__getitem__(0)
        var_16 = paddle.tensor.manipulation.expand(var_9, shape=[var_15, 1, 24, 36])
...
def create_inputspec():
    inputspec = (paddle.static.InputSpec(shape=(-1, -1, 24, 36), dtype=paddle.float32, stop_gradient=False),)
    return inputspec


def create_tensor_inputs():
    inputs = (paddle.rand(shape=[1, 256, 24, 36], dtype=paddle.float32),)
    return inputs


def create_numpy_inputs():
    inputs = (np.random.random(size=[1, 256, 24, 36]).astype("float32"),)
    return inputs
