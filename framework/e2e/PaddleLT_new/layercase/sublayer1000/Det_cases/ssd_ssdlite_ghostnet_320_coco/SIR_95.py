...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 1248, 10, 10], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_3 = paddle.nn.functional.common.linear(x=var_2, weight=self.parameter_3, bias=self.parameter_2, name=None)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[1, 1248, 10, 10], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 1248, 10, 10]).astype('float32'),
    )
    return inputs
