...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [70, 81], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [70], dtype: paddle.int64, stop_gradient: True)
    ):
        var_2 = paddle.tensor.creation.zeros([70, 81], dtype='float32')
...
def create_inputspec(): 
    inputspec = ( 
        paddle.static.InputSpec(shape=(-1, 81), dtype=paddle.float32, stop_gradient=False), 
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[70, 81], dtype=paddle.float32),
        paddle.randint(low=0, high=10, shape=[70], dtype=paddle.int64),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[70, 81]).astype('float32'),
        np.random.randint(low=0, high=10, size=[70], dtype='int64'),
    )
    return inputs
