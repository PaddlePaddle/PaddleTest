...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [247, 81], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [247], dtype: paddle.int64, stop_gradient: True)
    ):
        var_2 = paddle.tensor.creation.zeros([247, 81], dtype='float32')
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[247, 81]).astype('float32'),
