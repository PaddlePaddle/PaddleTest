...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 68, 100, 152], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = var_0.reshape([1, 4, -1, 100, 152])
        ...
        var_7 = var_6.reshape([1, 20, 100, 152])
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 68, 100, 152], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 68, 100, 152]).astype('float32'),
    )
    return inputs
