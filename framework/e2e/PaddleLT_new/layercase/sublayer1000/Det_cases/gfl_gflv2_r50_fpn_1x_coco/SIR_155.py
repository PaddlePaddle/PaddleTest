...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [3800, 81], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [3800], dtype: paddle.int64, stop_gradient: True)
    ):
        var_2 = paddle.tensor.creation.zeros([3800, 81], dtype='float32')
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[3800, 81], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[3800, 81]).astype('float32'),
