...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 4725], dtype: paddle.bool, stop_gradient: True)
        var_1,    # (shape: [1, 4725, 4], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 4725, 4], dtype: paddle.float32, stop_gradient: True)
        var_3,    # (shape: [1, 4725, 80], dtype: paddle.float32, stop_gradient: True)
        var_4,    # (shape: [], dtype: paddle.float32, stop_gradient: True)
        var_5,    # (shape: [1, 4725, 68], dtype: paddle.float32, stop_gradient: False)
        var_6,    # (shape: [4725, 2], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[1, 4725], dtype=paddle.bool),
        paddle.rand(shape=[1, 4725, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 4725, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 4725, 80], dtype=paddle.float32),
        paddle.rand(shape=[1], dtype=paddle.float32),
        paddle.rand(shape=[1, 4725, 68], dtype=paddle.float32),
        paddle.rand(shape=[4725, 2], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        ...
        np.random.random(size=[1, 4725, 4]).astype('float32'),
        np.random.random(size=[1, 4725, 80]).astype('float32'),
        np.random.random(size=[1]).astype('float32'),
        np.random.random(size=[1, 4725, 68]).astype('float32'),
        np.random.random(size=[4725, 2]).astype('float32'),
    )
    return inputs
