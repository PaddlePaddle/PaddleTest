...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 18, 176, 264], dtype: paddle.float32, stop_gradient: True)
        var_1,    # (shape: [1, 36, 88, 132], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 72, 44, 66], dtype: paddle.float32, stop_gradient: False)
        var_3,    # (shape: [1, 144, 22, 33], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_8 = paddle.nn.functional.conv._conv_nd(var_7, self.parameter_3, bias=None, stride=[1, 1], padding=[0, 0], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        ...
        var_14 = paddle.nn.functional.conv._conv_nd(var_9, self.parameter_5, bias=None, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        var_15 = paddle.nn.functional.conv._conv_nd(var_10, self.parameter_4, bias=None, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        ...
        var_17 = paddle.nn.functional.conv._conv_nd(var_12, self.parameter_2, bias=None, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 18, 176, 264]).astype('float32'),
        np.random.random(size=[1, 36, 88, 132]).astype('float32'),
        np.random.random(size=[1, 72, 44, 66]).astype('float32'),
        np.random.random(size=[1, 144, 22, 33]).astype('float32'),
    )
    return inputs
