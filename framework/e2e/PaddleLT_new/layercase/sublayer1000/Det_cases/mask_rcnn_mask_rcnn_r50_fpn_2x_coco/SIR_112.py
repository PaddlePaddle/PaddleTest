...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [3, 256, 28, 28], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [3], dtype: paddle.int32, stop_gradient: True)
        var_2,    # (shape: [3, 28, 28], dtype: paddle.int32, stop_gradient: True)
        var_3,    # (shape: [3], dtype: paddle.float32, stop_gradient: True)
    ):
        var_4 = paddle.nn.functional.conv._conv_nd(var_0, self.parameter_0, bias=self.parameter_1, stride=[1, 1], padding=[0, 0], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        ...
        var_10 = paddle.tensor.manipulation.reshape(var_9, [3, 28, 28])
...
def create_tensor_inputs():
    inputs = (
        ...
        paddle.randint(low=0, high=10, shape=[3], dtype=paddle.int32),
        paddle.randint(low=0, high=10, shape=[3, 28, 28], dtype=paddle.int32),
        paddle.rand(shape=[3], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[3, 256, 28, 28]).astype('float32'),
        np.random.randint(low=0, high=10, size=[3], dtype='int32'),
        np.random.randint(low=0, high=10, size=[3, 28, 28], dtype='int32'),
        np.random.random(size=[3]).astype('float32'),
    )
    return inputs
