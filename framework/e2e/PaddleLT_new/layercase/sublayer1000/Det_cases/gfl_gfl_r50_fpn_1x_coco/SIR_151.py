...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [3800], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [3800], dtype: paddle.float32, stop_gradient: True)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[3800], dtype=paddle.float32),
        paddle.rand(shape=[3800], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[3800]).astype('float32'),
        np.random.random(size=[3800]).astype('float32'),
