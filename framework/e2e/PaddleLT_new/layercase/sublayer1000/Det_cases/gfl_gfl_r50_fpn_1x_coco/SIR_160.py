...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [247], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [247], dtype: paddle.float32, stop_gradient: True)
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[247]).astype('float32'),
        np.random.random(size=[247]).astype('float32'),
