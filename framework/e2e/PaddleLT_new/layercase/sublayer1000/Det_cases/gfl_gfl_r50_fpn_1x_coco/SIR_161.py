...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [70, 80], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [70], dtype: paddle.int64, stop_gradient: True)
    ):
        var_2 = paddle.tensor.ops.sigmoid(var_0)
        var_3 = paddle.tensor.creation.zeros([70, 80], dtype='float32')
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[70, 80], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[70, 80]).astype('float32'),
