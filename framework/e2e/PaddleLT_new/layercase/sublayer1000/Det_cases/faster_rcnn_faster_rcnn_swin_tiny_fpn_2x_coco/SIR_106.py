...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [86970], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [256, 1], dtype: paddle.int64, stop_gradient: True)
        var_2,    # (shape: [86970], dtype: paddle.int32, stop_gradient: True)
        var_3,    # (shape: [6, 1], dtype: paddle.int64, stop_gradient: True)
        var_4,    # (shape: [86970, 4], dtype: paddle.float32, stop_gradient: False)
        var_5,    # (shape: [86970, 4], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[86970], dtype=paddle.float32),
        ...
        paddle.rand(shape=[86970, 4], dtype=paddle.float32),
        paddle.rand(shape=[86970, 4], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[86970]).astype('float32'),
        ...
        np.random.random(size=[86970, 4]).astype('float32'),
        np.random.random(size=[86970, 4]).astype('float32'),
    )
    return inputs
