...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,  # (shape: [1, 2, 3549], dtype: paddle.float32, stop_gradient: True)
        var_1,  # (shape: [1, 1], dtype: paddle.int32, stop_gradient: True)
        var_2,  # (shape: [1, 2, 1], dtype: paddle.int32, stop_gradient: True)
        var_3,  # (shape: [1, 3549], dtype: paddle.float32, stop_gradient: True)
        var_4,  # (shape: [1, 2, 4], dtype: paddle.float32, stop_gradient: True)
        var_5,  # (shape: [1, 2, 3549], dtype: paddle.float32, stop_gradient: True)
        var_6,  # (shape: [1, 2, 3549], dtype: paddle.float32, stop_gradient: True)
    ):
        var_1.stop_gradient = True
        var_2.stop_gradient = True
        var_4.stop_gradient = True
        var_7 = var_0.argmax(axis=-2)
        var_8 = var_1.__mul__(2)
...
def create_inputspec():
    inputspec = (
        ...
        paddle.static.InputSpec(shape=(-1, -1, -1), dtype=paddle.float32, stop_gradient=True),
        paddle.static.InputSpec(shape=(-1, -1, -1), dtype=paddle.float32, stop_gradient=False),
        paddle.static.InputSpec(shape=(-1, -1, -1), dtype=paddle.float32, stop_gradient=False),
    )
    return inputspec
