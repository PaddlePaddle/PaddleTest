...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 3549], dtype: paddle.bool, stop_gradient: True)
        var_1,    # (shape: [1, 3549, 4], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 3549, 4], dtype: paddle.float32, stop_gradient: True)
        var_3,    # (shape: [1, 3549, 80], dtype: paddle.float32, stop_gradient: True)
        var_4,    # (shape: [], dtype: paddle.float32, stop_gradient: True)
        var_5,    # (shape: [1, 3549, 76], dtype: paddle.float32, stop_gradient: False)
        var_6,    # (shape: [3549, 2], dtype: paddle.float32, stop_gradient: True)
    ):
        ...
        out = paddle.tensor.manipulation.split(var_12, num_or_sections=4, axis=-1)
        var_19 = out[0]
        var_20 = out[1]
        var_21 = out[2]
        var_22 = out[3]
        out = paddle.tensor.manipulation.split(var_14, num_or_sections=4, axis=-1)
        var_23 = out[0]
        var_24 = out[1]
        var_25 = out[2]
        var_26 = out[3]
        ...
        var_82 = var_77.__sub__(-2)
        var_83 = paddle.nn.functional.loss.cross_entropy(var_67, var_82, reduction='none')
        var_84 = var_83.__mul__(var_80)
        var_85 = var_78.__sub__(-2)
        var_86 = paddle.nn.functional.loss.cross_entropy(var_67, var_85, reduction='none')
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 3549], dtype=paddle.bool),
        paddle.rand(shape=[1, 3549, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 3549, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 3549, 80], dtype=paddle.float32),
        paddle.rand(shape=[1], dtype=paddle.float32),
        paddle.rand(shape=[1, 3549, 76], dtype=paddle.float32),
        paddle.rand(shape=[3549, 2], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 3549]).astype('bool'),
        np.random.random(size=[1, 3549, 4]).astype('float32'),
        np.random.random(size=[1, 3549, 4]).astype('float32'),
        np.random.random(size=[1, 3549, 80]).astype('float32'),
        np.random.random(size=[1]).astype('float32'),
        np.random.random(size=[1, 3549, 76]).astype('float32'),
        np.random.random(size=[3549, 2]).astype('float32'),
    )
    return inputs
