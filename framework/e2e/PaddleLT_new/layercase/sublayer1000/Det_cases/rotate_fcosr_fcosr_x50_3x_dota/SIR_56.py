...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 256, 32, 32], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [1], dtype: paddle.float32, stop_gradient: True)
    ):
        var_2 = paddle.nn.functional.conv._conv_nd(var_0, self.parameter_6, bias=self.parameter_3, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        var_3 = var_2.__mul__(self.parameter_0)
        var_4 = var_3.__mul__(32)
        var_5 = paddle.nn.functional.conv._conv_nd(var_0, self.parameter_5, bias=self.parameter_1, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        var_6 = var_5.__mul__(self.parameter_0)
        var_7 = var_6.__add__(1.0)
        var_8 = paddle.nn.functional.activation.elu(var_7)
        var_9 = var_8.__mul__(32)
        var_10 = paddle.nn.functional.conv._conv_nd(var_0, self.parameter_2, bias=self.parameter_4, stride=[1, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
