...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [390, 64, 7, 7], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = paddle.tensor.manipulation.flatten(var_0, start_axis=1, stop_axis=-1)
        var_2 = paddle.nn.functional.common.linear(x=var_1, weight=self.parameter_2, bias=self.parameter_3, name=None)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[390, 64, 7, 7], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[390, 64, 7, 7]).astype('float32'),
    )
    return inputs
