...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [6, 1, 1], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [6, 1, 1], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [6, 1, 1], dtype: paddle.float32, stop_gradient: False)
        var_3,    # (shape: [6, 1, 1], dtype: paddle.float32, stop_gradient: False)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[6, 1, 1], dtype=paddle.float32),
        paddle.rand(shape=[6, 1, 1], dtype=paddle.float32),
        paddle.rand(shape=[6, 1, 1], dtype=paddle.float32),
        paddle.rand(shape=[6, 1, 1], dtype=paddle.float32),
    )
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[6, 1, 1]).astype('float32'),
        np.random.random(size=[6, 1, 1]).astype('float32'),
        np.random.random(size=[6, 1, 1]).astype('float32'),
        np.random.random(size=[6, 1, 1]).astype('float32'),
    )
