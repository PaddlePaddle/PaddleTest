...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [220968], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [256, 1], dtype: paddle.int64, stop_gradient: True)
        var_2,    # (shape: [220968], dtype: paddle.int32, stop_gradient: True)
        var_3,    # (shape: [5, 1], dtype: paddle.int64, stop_gradient: True)
        var_4,    # (shape: [220968, 4], dtype: paddle.float32, stop_gradient: False)
        var_5,    # (shape: [220968, 4], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[220968]).astype('float32'),
        np.random.randint(low=0, high=10, size=[256, 1], dtype='int64'),
        np.random.randint(low=0, high=10, size=[220968], dtype='int32'),
        np.random.randint(low=0, high=10, size=[5, 1], dtype='int64'),
        np.random.random(size=[220968, 4]).astype('float32'),
        np.random.random(size=[220968, 4]).astype('float32'),
    )
    return inputs
