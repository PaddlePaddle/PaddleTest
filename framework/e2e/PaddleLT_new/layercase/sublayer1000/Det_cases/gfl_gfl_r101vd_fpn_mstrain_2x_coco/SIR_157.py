...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [150, 80], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [150], dtype: paddle.int64, stop_gradient: True)
    ):
        var_2 = paddle.tensor.ops.sigmoid(var_0)
        var_3 = paddle.tensor.creation.zeros([150, 80], dtype='float32')
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[150, 80], dtype=paddle.float32),
        paddle.randint(low=0, high=10, shape=[150], dtype=paddle.int64),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[150, 80]).astype('float32'),
        np.random.randint(low=0, high=10, size=[150], dtype='int64'),
    )
    return inputs
