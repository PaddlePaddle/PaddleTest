...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [150], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [150], dtype: paddle.float32, stop_gradient: True)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[150], dtype=paddle.float32),
        paddle.rand(shape=[150], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[150]).astype('float32'),
        np.random.random(size=[150]).astype('float32'),
