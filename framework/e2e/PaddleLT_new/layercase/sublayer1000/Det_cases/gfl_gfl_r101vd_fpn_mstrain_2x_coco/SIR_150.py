...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1,    # (shape: [8816], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [8816], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        ...
        paddle.rand(shape=[8816], dtype=paddle.float32),
        paddle.rand(shape=[8816], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        ...
        np.random.random(size=[8816]).astype('float32'),
        np.random.random(size=[8816]).astype('float32'),
    )
    return inputs
