...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [20, 8], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [20], dtype: paddle.float32, stop_gradient: True)
        var_2,    # (shape: [20], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[20, 8], dtype=paddle.float32),
        paddle.rand(shape=[20], dtype=paddle.float32),
        paddle.rand(shape=[20], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[20, 8]).astype('float32'),
        np.random.random(size=[20]).astype('float32'),
        np.random.random(size=[20]).astype('float32'),
    )
    return inputs
