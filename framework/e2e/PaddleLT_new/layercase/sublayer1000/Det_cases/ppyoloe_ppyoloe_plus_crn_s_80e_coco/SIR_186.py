...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 6069], dtype: paddle.bool, stop_gradient: True)
        var_1,    # (shape: [1, 6069, 4], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 6069, 4], dtype: paddle.float32, stop_gradient: True)
        var_3,    # (shape: [1, 6069, 80], dtype: paddle.float32, stop_gradient: True)
        var_4,    # (shape: [], dtype: paddle.float32, stop_gradient: True)
        var_5,    # (shape: [1, 6069, 68], dtype: paddle.float32, stop_gradient: False)
        var_6,    # (shape: [6069, 2], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 6069], dtype=paddle.bool),
        paddle.rand(shape=[1, 6069, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 6069, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 6069, 80], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 6069]).astype('bool'),
        np.random.random(size=[1, 6069, 4]).astype('float32'),
        np.random.random(size=[1, 6069, 4]).astype('float32'),
        np.random.random(size=[1, 6069, 80]).astype('float32'),
