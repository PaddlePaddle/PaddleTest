...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 96, 68, 68], dtype: paddle.float32, stop_gradient: False)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[1, 96, 68, 68], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 96, 68, 68]).astype('float32'),
