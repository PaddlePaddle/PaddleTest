...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 96, 30, 30], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [1, 96, 30, 30], dtype: paddle.float32, stop_gradient: False)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 96, 30, 30], dtype=paddle.float32),
        paddle.rand(shape=[1, 96, 30, 30], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 96, 30, 30]).astype('float32'),
        np.random.random(size=[1, 96, 30, 30]).astype('float32'),
    )
    return inputs
