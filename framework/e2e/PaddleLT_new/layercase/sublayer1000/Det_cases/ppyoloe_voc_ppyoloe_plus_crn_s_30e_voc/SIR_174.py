...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1,    # (shape: [1, 20, 40, 40], dtype: paddle.float32, stop_gradient: False)
    ):
...
def create_paddle_inputs():
    inputs = (
        ...
        paddle.rand(shape=[1, 20, 40, 40], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        ...
        np.random.random(size=[1, 20, 40, 40]).astype('float32'),
    )
    return inputs
