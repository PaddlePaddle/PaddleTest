...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 2100], dtype: paddle.bool, stop_gradient: True)
        var_1,    # (shape: [1, 2100, 4], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 2100, 4], dtype: paddle.float32, stop_gradient: True)
        var_3,    # (shape: [1, 2100, 20], dtype: paddle.float32, stop_gradient: True)
        var_4,    # (shape: [], dtype: paddle.float32, stop_gradient: True)
        var_5,    # (shape: [1, 2100, 68], dtype: paddle.float32, stop_gradient: False)
        var_6,    # (shape: [2100, 2], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 2100], dtype=paddle.bool),
        paddle.rand(shape=[1, 2100, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 2100, 4], dtype=paddle.float32),
        paddle.rand(shape=[1, 2100, 20], dtype=paddle.float32),
        paddle.rand(shape=[1], dtype=paddle.float32),
        paddle.rand(shape=[1, 2100, 68], dtype=paddle.float32),
        paddle.rand(shape=[2100, 2], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 2100]).astype('bool'),
        np.random.random(size=[1, 2100, 4]).astype('float32'),
        np.random.random(size=[1, 2100, 4]).astype('float32'),
        np.random.random(size=[1, 2100, 20]).astype('float32'),
        np.random.random(size=[1]).astype('float32'),
        np.random.random(size=[1, 2100, 68]).astype('float32'),
        np.random.random(size=[2100, 2]).astype('float32'),
    )
    return inputs
