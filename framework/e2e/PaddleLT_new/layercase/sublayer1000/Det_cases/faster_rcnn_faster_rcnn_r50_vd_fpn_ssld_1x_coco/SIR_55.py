...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [153450], dtype: paddle.int32, stop_gradient: True)
        var_1,    # (shape: [1, 153450, 1], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 153450, 4], dtype: paddle.float32, stop_gradient: False)
    ):
...
def create_tensor_inputs():
    inputs = (
        ...
        paddle.rand(shape=[1, 153450, 1], dtype=paddle.float32),
        paddle.rand(shape=[1, 153450, 4], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        ...
        np.random.random(size=[1, 153450, 1]).astype('float32'),
        np.random.random(size=[1, 153450, 4]).astype('float32'),
    )
    return inputs
