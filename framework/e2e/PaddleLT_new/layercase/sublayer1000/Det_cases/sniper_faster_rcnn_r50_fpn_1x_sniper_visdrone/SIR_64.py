...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1021], dtype: paddle.int64, stop_gradient: True)
        var_1,    # (shape: [1021], dtype: paddle.int32, stop_gradient: True)
        var_2,    # (shape: [21], dtype: paddle.int32, stop_gradient: True)
    ):
        var_3 = paddle.tensor.manipulation.gather(var_2, var_0)
        var_4 = var_1.__eq__(0)
        var_5 = paddle.tensor.creation.ones_like(var_3)
        var_6 = var_5.__mul__(9)
...
def create_tensor_inputs():
    inputs = (
        paddle.randint(low=0, high=10, shape=[1021], dtype=paddle.int64),
        paddle.randint(low=0, high=10, shape=[1021], dtype=paddle.int32),
        paddle.randint(low=0, high=10, shape=[21], dtype=paddle.int32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.randint(low=0, high=10, size=[1021], dtype='int64'),
        np.random.randint(low=0, high=10, size=[1021], dtype='int32'),
        np.random.randint(low=0, high=10, size=[21], dtype='int32'),
    )
    return inputs
