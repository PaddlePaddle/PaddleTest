...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [217413], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [256, 1], dtype: paddle.int64, stop_gradient: True)
        var_2,    # (shape: [217413], dtype: paddle.int32, stop_gradient: True)
        var_3,    # (shape: [103, 1], dtype: paddle.int64, stop_gradient: True)
        var_4,    # (shape: [217413, 4], dtype: paddle.float32, stop_gradient: False)
        var_5,    # (shape: [217413, 4], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[217413], dtype=paddle.float32),
        paddle.randint(low=0, high=10, shape=[256, 1], dtype=paddle.int64),
        paddle.randint(low=0, high=10, shape=[217413], dtype=paddle.int32),
        paddle.randint(low=0, high=10, shape=[103, 1], dtype=paddle.int64),
        paddle.rand(shape=[217413, 4], dtype=paddle.float32),
        paddle.rand(shape=[217413, 4], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[217413]).astype('float32'),
        np.random.randint(low=0, high=10, size=[256, 1], dtype='int64'),
        np.random.randint(low=0, high=10, size=[217413], dtype='int32'),
        np.random.randint(low=0, high=10, size=[103, 1], dtype='int64'),
        np.random.random(size=[217413, 4]).astype('float32'),
        np.random.random(size=[217413, 4]).astype('float32'),
    )
    return inputs
