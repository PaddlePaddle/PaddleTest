...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,  # (shape: [1, 8732, 4], dtype: paddle.float32, stop_gradient: False)
        var_1,  # (shape: [1, 8732, 4], dtype: paddle.bool, stop_gradient: True)
        var_2,  # (shape: [1, 8732, 4], dtype: paddle.float32, stop_gradient: True)
        var_3,  # (shape: [1, 8732, 21], dtype: paddle.float32, stop_gradient: False)
        var_4,  # (shape: [1, 8732, 1], dtype: paddle.int64, stop_gradient: True)
    ):
...
def create_inputspec():
    inputspec = (
        ...
        paddle.static.InputSpec(shape=(-1, -1, -1), dtype=paddle.float32, stop_gradient=False),
        paddle.static.InputSpec(shape=(-1, -1, -1), dtype=paddle.float32, stop_gradient=False),
        paddle.static.InputSpec(shape=(-1, -1, 1), dtype=paddle.int64, stop_gradient=False),
    )
    return inputspec
...
def create_tensor_inputs():
    inputs = (
        ...
        paddle.randint(low=0, high=10, shape=[1, 8732, 1], dtype=paddle.int64),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 8732, 4]).astype("float32"),
        np.random.random(size=[1, 8732, 4]).astype("bool"),
        np.random.random(size=[1, 8732, 4]).astype("float32"),
        np.random.random(size=[1, 8732, 21]).astype("float32"),
        np.random.randint(low=0, high=10, size=[1, 8732, 1], dtype="int64"),
    )
    return inputs
