...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 8732], dtype: paddle.bool, stop_gradient: True)
        var_1,    # (shape: [1, 8732, 1], dtype: paddle.float32, stop_gradient: False)
        var_2,    # (shape: [1, 8732, 1], dtype: paddle.int64, stop_gradient: True)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 8732], dtype=paddle.bool),
        paddle.rand(shape=[1, 8732, 1], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 8732]).astype('bool'),
        np.random.random(size=[1, 8732, 1]).astype('float32'),
