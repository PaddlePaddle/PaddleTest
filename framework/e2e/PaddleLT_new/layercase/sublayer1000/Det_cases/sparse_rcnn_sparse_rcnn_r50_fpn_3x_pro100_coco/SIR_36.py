...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_4,    # (shape: [1, 100, 4], dtype: paddle.float32, stop_gradient: False)
...
def create_numpy_inputs():
    inputs = (
        ...
        np.random.random(size=[1, 100, 4]).astype('float32'),
        np.random.random(size=[1, 100, 256]).astype('float32'),
    )
    return inputs
