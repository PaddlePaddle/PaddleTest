...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1,    # (shape: [1, 1, 320, 320], dtype: paddle.float32, stop_gradient: True)
    ):
        paddle.seed(33)
        var_2 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[128], weight=self.parameter_0, bias=self.parameter_3, epsilon=1e-06)
        ...
        var_18 = paddle.nn.functional.common.linear(x=var_17, weight=self.parameter_4, bias=self.parameter_5, name=None)
...
def create_inputspec(): 
    inputspec = ( 
        paddle.static.InputSpec(shape=(-1, -1, 128), dtype=paddle.float32, stop_gradient=False), 
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[10, 320, 128], dtype=paddle.float32),
        paddle.rand(shape=[1, 1, 320, 320], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[10, 320, 128]).astype('float32'),
        np.random.random(size=[1, 1, 320, 320]).astype('float32'),
    )
    return inputs
