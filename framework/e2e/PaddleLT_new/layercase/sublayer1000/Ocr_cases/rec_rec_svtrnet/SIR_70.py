...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_2 = var_1.reshape([0, 128, 4, 25])
        var_3 = paddle.nn.functional.conv._conv_nd(var_2, self.parameter_3, bias=self.parameter_0, stride=[2, 1], padding=[1, 1], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        var_4 = var_3.flatten(2)
        var_5 = var_4.transpose((0, 2, 1,))
        var_6 = paddle.nn.functional.norm.layer_norm(var_5, normalized_shape=[256], weight=self.parameter_1, bias=self.parameter_2, epsilon=1e-05)
        return var_6
