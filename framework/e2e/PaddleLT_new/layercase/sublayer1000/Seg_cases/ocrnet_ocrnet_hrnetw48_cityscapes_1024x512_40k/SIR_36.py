...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 512, 128, 256], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [1, 19, 128, 256], dtype: paddle.float32, stop_gradient: False)
    ):
        var_2 = paddle.tensor.manipulation.reshape(var_0, (0, 512, -1,))
        var_3 = paddle.tensor.linalg.transpose(var_2, (0, 2, 1,))
        var_4 = paddle.tensor.manipulation.reshape(var_1, (0, 19, -1,))
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 512, 128, 256], dtype=paddle.float32),
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 512, 128, 256]).astype('float32'),
