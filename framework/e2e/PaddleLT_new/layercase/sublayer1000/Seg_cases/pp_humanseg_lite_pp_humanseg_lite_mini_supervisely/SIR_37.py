...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 72, 14, 25], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [1, 72, 14, 25], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_6 = paddle.tensor.manipulation.reshape(x=var_2, shape=[0, 2, 72, var_4, var_5])
        var_7 = paddle.tensor.linalg.transpose(x=var_6, perm=[0, 2, 1, 3, 4])
        var_8 = paddle.tensor.manipulation.reshape(x=var_7, shape=[0, 144, var_4, var_5])
        return var_8


def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[1, 72, 14, 25], dtype=paddle.float32),
        paddle.rand(shape=[1, 72, 14, 25], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 72, 14, 25]).astype('float32'),
        np.random.random(size=[1, 72, 14, 25]).astype('float32'),
    )
    return inputs
