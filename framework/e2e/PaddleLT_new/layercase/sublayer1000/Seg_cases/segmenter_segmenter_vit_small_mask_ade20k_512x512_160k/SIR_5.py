...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 1025, 384], dtype: paddle.float32, stop_gradient: False)
    ):
        paddle.seed(33)
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[384], weight=self.parameter_0, bias=self.parameter_1, epsilon=1e-05)
        var_2 = var_1.shape
        var_3 = var_2.__getitem__(1)
        var_4 = var_2.__getitem__(2)
        var_5 = paddle.nn.functional.common.linear(x=var_1, weight=self.parameter_2, bias=self.parameter_4, name=None)
        var_6 = var_4.__floordiv__(6)
        var_7 = var_5.reshape((-1, var_3, 3, 6, var_6,))
        ...
        var_20 = paddle.nn.functional.common.linear(x=var_19, weight=self.parameter_5, bias=self.parameter_3, name=None)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 1025, 384], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 1025, 384]).astype('float32'),
    )
    return inputs
