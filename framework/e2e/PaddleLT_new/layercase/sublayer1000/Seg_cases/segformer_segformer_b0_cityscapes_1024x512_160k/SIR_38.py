...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [1, 512, 256], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[256], weight=self.parameter_4, bias=self.parameter_6, epsilon=1e-06)
        var_2 = var_1.shape
        var_3 = var_2.__getitem__(0)
        var_4 = var_2.__getitem__(1)
        var_5 = paddle.nn.functional.common.linear(x=var_1, weight=self.parameter_7, bias=self.parameter_3, name=None)
        var_6 = var_5.reshape([var_3, var_4, 8, 32])
        var_7 = var_6.transpose([0, 2, 1, 3])
        var_8 = paddle.nn.functional.common.linear(x=var_1, weight=self.parameter_5, bias=self.parameter_2, name=None)
        ...
        var_21 = paddle.nn.functional.common.linear(x=var_20, weight=self.parameter_1, bias=self.parameter_0, name=None)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[1, 512, 256], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 512, 256]).astype('float32'),
    )
    return inputs
