...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [11, 2048, 7, 7], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[11, 2048, 7, 7], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[11, 2048, 7, 7]).astype('float32'),
    )
    return inputs
