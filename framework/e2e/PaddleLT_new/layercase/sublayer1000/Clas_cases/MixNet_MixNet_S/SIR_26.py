...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [22, 72, 56, 56], dtype: paddle.float32, stop_gradient: False)
    ):
        out = paddle.tensor.manipulation.split(var_0, [36, 36], axis=1)
        var_1 = out[0]
        var_2 = out[1]
        out = paddle.tensor.manipulation.split(var_0, [36, 36], axis=1)
        var_3 = out[0]
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[22, 72, 56, 56], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[22, 72, 56, 56]).astype('float32'),
    )
    return inputs
