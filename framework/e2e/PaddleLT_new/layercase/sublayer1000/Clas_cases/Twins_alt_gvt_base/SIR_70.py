...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [43, 384, 14, 14], dtype: paddle.float32, stop_gradient: False)
    ):
        paddle.seed(123)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[43, 384, 14, 14], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[43, 384, 14, 14]).astype('float32'),
    )
    return inputs
