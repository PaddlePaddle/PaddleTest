...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [11, 784, 192], dtype: paddle.float32, stop_gradient: True)
    ):
        var_1 = paddle.nn.functional.common.linear(x=var_0, weight=self.parameter_0, bias=self.parameter_5, name=None)
        var_2 = var_1.reshape([11, 784, 6, 32])
        var_3 = var_2.transpose([0, 2, 1, 3])
        var_4 = var_0.transpose([0, 2, 1])
        var_5 = var_4.reshape([11, 192, 28, 28])
        var_6 = paddle.nn.functional.conv._conv_nd(var_5, self.parameter_2, bias=self.parameter_1, stride=[4, 4], padding=[0, 0], padding_algorithm='EXPLICIT', dilation=[1, 1], groups=1, data_format='NCHW', channel_dim=1, op_type='conv2d', use_cudnn=True)
        var_7 = var_6.reshape([11, 192, 49])
        var_8 = var_7.transpose([0, 2, 1])
        var_9 = paddle.nn.functional.norm.layer_norm(var_8, normalized_shape=[192], weight=self.parameter_6, bias=self.parameter_3, epsilon=1e-05)
        var_10 = paddle.nn.functional.common.linear(x=var_9, weight=self.parameter_7, bias=self.parameter_4, name=None)
