...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [10, 197, 384], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = paddle.nn.functional.common.linear(x=var_0, weight=self.parameter_0, bias=None, name=None)
        var_2 = var_1.reshape((10, 197, 2, 6, 64,))
        ...
        var_7 = var_6.reshape((10, 197, 6, 64,))
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[10, 197, 384], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[10, 197, 384]).astype('float32'),
    )
    return inputs
