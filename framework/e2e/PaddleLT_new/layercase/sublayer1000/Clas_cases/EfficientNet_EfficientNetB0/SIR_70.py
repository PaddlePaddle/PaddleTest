...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [43, 80, 14, 14], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [43, 80, 14, 14], dtype: paddle.float32, stop_gradient: False)
    ):
        var_2 = var_0.shape
        var_3 = var_2.__getitem__(0)
        var_4 = paddle.tensor.random.rand(shape=[var_3, 1, 1, 1])
        var_5 = var_4.__radd__(0.9125)
        var_6 = paddle.tensor.ops.floor(var_5)
        var_7 = paddle.tensor.math.multiply(var_0, var_6)
        var_8 = var_7.__truediv__(0.9125)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[43, 80, 14, 14], dtype=paddle.float32),
        paddle.rand(shape=[43, 80, 14, 14], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[43, 80, 14, 14]).astype('float32'),
        np.random.random(size=[43, 80, 14, 14]).astype('float32'),
    )
    return inputs
