...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [43, 24, 56, 56], dtype: paddle.float32, stop_gradient: False)
        var_1,    # (shape: [43, 24, 56, 56], dtype: paddle.float32, stop_gradient: False)
    ):
        var_2 = var_0.shape
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[43, 24, 56, 56], dtype=paddle.float32),
        paddle.rand(shape=[43, 24, 56, 56], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[43, 24, 56, 56]).astype('float32'),
        np.random.random(size=[43, 24, 56, 56]).astype('float32'),
    )
    return inputs
