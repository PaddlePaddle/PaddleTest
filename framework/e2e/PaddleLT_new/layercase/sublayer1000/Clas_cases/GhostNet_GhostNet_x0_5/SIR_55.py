...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [145, 336, 14, 14], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_3 = paddle.nn.functional.common.linear(x=var_2, weight=self.parameter_2, bias=self.parameter_3, name=None)
        var_4 = paddle.nn.functional.activation.relu(var_3)
        var_5 = paddle.nn.functional.common.linear(x=var_4, weight=self.parameter_0, bias=self.parameter_1, name=None)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[145, 336, 14, 14], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[145, 336, 14, 14]).astype('float32'),
    )
    return inputs
