...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [22, 60, 28, 28], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[22, 60, 28, 28], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[22, 60, 28, 28]).astype('float32'),
    )
    return inputs
