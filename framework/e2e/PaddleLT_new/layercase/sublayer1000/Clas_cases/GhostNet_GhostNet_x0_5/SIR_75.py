...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [22, 336, 14, 14], dtype: paddle.float32, stop_gradient: True)
    ):
        ...
        var_3 = paddle.nn.functional.common.linear(x=var_2, weight=self.parameter_0, bias=self.parameter_3, name=None)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[22, 336, 14, 14], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[22, 336, 14, 14]).astype('float32'),
    )
    return inputs
