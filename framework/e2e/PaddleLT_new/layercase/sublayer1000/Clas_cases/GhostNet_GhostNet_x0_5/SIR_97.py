...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [10, 336, 7, 7], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[10, 336, 7, 7], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[10, 336, 7, 7]).astype('float32'),
    )
    return inputs
