...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [145, 36, 28, 28], dtype: paddle.float32, stop_gradient: False)
    ):
        ...
        var_5 = paddle.nn.functional.common.linear(x=var_4, weight=self.parameter_1, bias=self.parameter_0, name=None)
...
def create_paddle_inputs():
    inputs = (
        paddle.rand(shape=[145, 36, 28, 28], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[145, 36, 28, 28]).astype('float32'),
    )
    return inputs
