...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [6, 2304, 192], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[192], weight=self.parameter_1, bias=self.parameter_2, epsilon=1e-05)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[6, 2304, 192], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[6, 2304, 192]).astype('float32'),
    )
    return inputs
