...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[96], weight=self.parameter_1, bias=self.parameter_0, epsilon=1e-05)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[6, 9216, 96], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[6, 9216, 96]).astype('float32'),
    )
    return inputs
