...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [960, 2, 96, 24], dtype: paddle.float32, stop_gradient: True)
        var_1,    # (shape: [960, 2, 96, 24], dtype: paddle.float32, stop_gradient: True)
    ):
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[960, 2, 96, 24]).astype('float32'),
        np.random.random(size=[960, 2, 96, 24]).astype('float32'),
    )
    return inputs
