...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_1 = paddle.nn.functional.norm.layer_norm(var_0, normalized_shape=[768], weight=self.parameter_2, bias=self.parameter_3, epsilon=1e-05)
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[4, 144, 768], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[4, 144, 768]).astype('float32'),
    )
    return inputs
