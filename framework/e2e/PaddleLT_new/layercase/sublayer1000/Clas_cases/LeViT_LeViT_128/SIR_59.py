...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_0,    # (shape: [22, 49, 1280], dtype: paddle.float32, stop_gradient: False)
    ):
        var_1 = paddle.tensor.manipulation.reshape(var_0, [22, 49, 16, -1])
...
def create_tensor_inputs():
    inputs = (
        paddle.rand(shape=[22, 49, 1280], dtype=paddle.float32),
    )
    return inputs
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[22, 49, 1280]).astype('float32'),
    )
    return inputs
