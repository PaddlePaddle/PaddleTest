...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(self, x, ):
        ...
        forward
        """
        out = paddle.nn.functional.conv3d(x,  weight=paddle.to_tensor(-1 + (1 - -1) * np.random.random([1, 3, 3, 3, 3]).astype('float32'), dtype='float32', stop_gradient=False), stride=[2, 2, 1], padding=1, dilation=2, )
        return out
