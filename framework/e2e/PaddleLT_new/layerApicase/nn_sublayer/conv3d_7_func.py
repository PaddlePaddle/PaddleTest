...
def create_tensor_inputs():
    ...
    paddle tensor
    """
    inputs = (paddle.to_tensor(-1 + (1 - -1) * np.random.random([2, 4, 4, 4, 3]).astype('float32'), dtype='float32', stop_gradient=False), )
    return inputs
...
def create_numpy_inputs():
    ...
    numpy array
    """
    inputs = (-1 + (1 - -1) * np.random.random([2, 4, 4, 4, 3]).astype('float32'), )
    return inputs
