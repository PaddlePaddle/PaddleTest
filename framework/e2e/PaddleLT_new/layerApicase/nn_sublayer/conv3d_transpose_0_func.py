...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(self, x, ):
        ...
        paddle.seed(33)
        np.random.seed(33)
        out = paddle.nn.functional.conv3d_transpose(x,  weight=paddle.to_tensor(-1 + (1 - -1) * np.random.random([3, 1, 3, 3, 3]).astype('float32'), dtype='float32', stop_gradient=False), stride=1, padding=0, output_padding=0, dilation=1, data_format='NCDHW', )
        return out
