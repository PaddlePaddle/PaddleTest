...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(self, x, ):
        ...
        forward
        """
        out = paddle.nn.functional.conv2d(x,  weight=paddle.to_tensor(-1 + (1 - -1) * np.random.random([6, 1, 3, 3]).astype('float32'), dtype='float32', stop_gradient=False), stride=2, padding=0, groups=3, )
        return out
