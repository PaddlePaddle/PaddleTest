...
def NumOperationsInBlock(block_idx):
    return [64, 147][block_idx] - 1 # number-of-ops-in-block
...
class BlockEntries:
    def pd_op_while_932_0_0(self, constant_0, parameter_0, constant_1, transpose_0, parameter_1, parameter_2, parameter_3, constant_2, parameter_4, constant_3, parameter_5, parameter_6, parameter_7, parameter_8, parameter_9, parameter_10, constant_4, parameter_11, less_than_1, full_with_tensor_0, assign_value_0, full_with_tensor_1, full_1, full_with_tensor_2, full_0, assign_value_2, assign_value_1, assign_value_4, assign_value_3):

        # pd_op.scale: (xi64) <- (xi64, 1xf32)
        scale_0 = paddle._C_ops.scale(full_0, constant_0, float('1'), True)
        ...
        slice_0 = paddle._C_ops.slice(parameter_0, [0], combine_0, combine_1, [-1], [0])

        # pd_op.one_hot: (-1x30xf32) <- (-1xi32, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(full_with_tensor_0 % paddle.cast(constant_1, full_with_tensor_0.dtype), constant_1)

        # pd_op.matmul: (-1x256x256xf16) <- (-1x256x576xf16, 576x256xf16)
        matmul_0 = paddle.matmul(transpose_0, parameter_1, transpose_x=False, transpose_y=False)

        # pd_op.matmul: (-1x256xf16) <- (-1x256xf16, 256x256xf16)
        matmul_1 = paddle.matmul(full_with_tensor_1, parameter_2, transpose_x=False, transpose_y=False)

        # pd_op.add_: (-1x256xf16) <- (-1x256xf16, 256xf16)
        add__0 = paddle._C_ops.add(matmul_1, parameter_3)

        # pd_op.unsqueeze_: (-1x1x256xf16, None) <- (-1x256xf16, 1xi64)
        unsqueeze__0, unsqueeze__1 = (lambda x, f: f(x))(paddle._C_ops.unsqueeze(add__0, constant_2), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.add_: (-1x256x256xf16) <- (-1x256x256xf16, -1x1x256xf16)
        add__1 = paddle._C_ops.add(matmul_0, unsqueeze__0)

        # pd_op.tanh_: (-1x256x256xf16) <- (-1x256x256xf16)
        tanh__0 = paddle._C_ops.tanh(add__1)

        # pd_op.matmul: (-1x256x1xf16) <- (-1x256x256xf16, 256x1xf16)
        matmul_2 = paddle.matmul(tanh__0, parameter_4, transpose_x=False, transpose_y=False)

        # pd_op.softmax_: (-1x256x1xf16) <- (-1x256x1xf16)
        softmax__0 = paddle._C_ops.softmax(matmul_2, 1)

        # pd_op.transpose: (-1x1x256xf16) <- (-1x256x1xf16)
        transpose_1 = paddle._C_ops.transpose(softmax__0, [0, 2, 1])

        # pd_op.matmul: (-1x1x576xf16) <- (-1x1x256xf16, -1x256x576xf16)
        matmul_3 = paddle.matmul(transpose_1, transpose_0, transpose_x=False, transpose_y=False)

        # pd_op.squeeze_: (-1x576xf16, None) <- (-1x1x576xf16, 1xi64)
        squeeze__0, squeeze__1 = (lambda x, f: f(x))(paddle._C_ops.squeeze(matmul_3, constant_2), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.cast: (-1x576xf32) <- (-1x576xf16)
        ...
        concat_0 = paddle._C_ops.concat(combine_2, constant_3)

        # pd_op.cast: (-1x606xf16) <- (-1x606xf32)
        cast_1 = paddle._C_ops.cast(concat_0, paddle.float16)

        # pd_op.matmul: (-1x768xf16) <- (-1x606xf16, 768x606xf16)
        matmul_4 = paddle.matmul(cast_1, parameter_5, transpose_x=False, transpose_y=True)

        # pd_op.add_: (-1x768xf16) <- (-1x768xf16, 768xf16)
        add__2 = paddle._C_ops.add(matmul_4, parameter_6)

        # pd_op.matmul: (-1x768xf16) <- (-1x256xf16, 768x256xf16)
        matmul_5 = paddle.matmul(full_with_tensor_1, parameter_7, transpose_x=False, transpose_y=True)

        # pd_op.add_: (-1x768xf16) <- (-1x768xf16, 768xf16)
        add__3 = paddle._C_ops.add(matmul_5, parameter_8)

        # pd_op.split_with_num: ([-1x256xf16, -1x256xf16, -1x256xf16]) <- (-1x768xf16, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(add__2, 3, constant_3)

        # pd_op.split_with_num: ([-1x256xf16, -1x256xf16, -1x256xf16]) <- (-1x768xf16, 1xi32)
        split_with_num_1 = paddle._C_ops.split_with_num(add__3, 3, constant_3)
        ...
        add__4 = paddle._C_ops.add(slice_1, slice_2)

        # pd_op.sigmoid_: (-1x256xf16) <- (-1x256xf16)
        sigmoid__0 = paddle._C_ops.sigmoid(add__4)
        ...
        # pd_op.add_: (-1x256xf16) <- (-1x256xf16, -1x256xf16)
        add__5 = paddle._C_ops.add(slice_3, slice_4)
        ...
        multiply__0 = paddle._C_ops.multiply(sigmoid__0, slice_5)

        # builtin.slice: (-1x256xf16) <- ([-1x256xf16, -1x256xf16, -1x256xf16])
        slice_6 = split_with_num_0[2]

        # pd_op.add_: (-1x256xf16) <- (-1x256xf16, -1x256xf16)
        add__6 = paddle._C_ops.add(slice_6, multiply__0)

        # pd_op.tanh_: (-1x256xf16) <- (-1x256xf16)
        tanh__1 = paddle._C_ops.tanh(add__6)

        # pd_op.subtract: (-1x256xf16) <- (-1x256xf16, -1x256xf16)
        subtract_0 = full_with_tensor_1 - tanh__1

        # pd_op.multiply_: (-1x256xf16) <- (-1x256xf16, -1x256xf16)
        multiply__1 = paddle._C_ops.multiply(subtract_0, sigmoid__1)

        # pd_op.add_: (-1x256xf16) <- (-1x256xf16, -1x256xf16)
        add__7 = paddle._C_ops.add(multiply__1, tanh__1)

        # pd_op.scale: (xi64) <- (xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(slice_0, constant_0, float('1'), True)

        # pd_op.cast: (-1x256xf32) <- (-1x256xf16)
        cast_2 = paddle._C_ops.cast(add__7, paddle.float32)

        # pd_op.cast: (-1x256xf16) <- (-1x256xf32)
        ...
        # pd_op.set_value_with_tensor: (-1x501x256xf16) <- (-1x501x256xf16, -1x256xf16, [xi64], [xi64], 1xi64)
        set_value_with_tensor_0 = paddle._C_ops.set_value_with_tensor(full_with_tensor_2, cast_3, combine_3, combine_4, constant_2, [1], [1], [])

        # pd_op.matmul: (-1x30xf16) <- (-1x256xf16, 256x30xf16)
        matmul_6 = paddle.matmul(add__7, parameter_9, transpose_x=False, transpose_y=False)

        # pd_op.add_: (-1x30xf16) <- (-1x30xf16, 30xf16)
        add__8 = paddle._C_ops.add(matmul_6, parameter_10)

        # pd_op.argmax: (-1xi32) <- (-1x30xf16, 1xi64)
        argmax_0 = paddle._C_ops.argmax(add__8, constant_4, False, False, paddle.int32)

        # pd_op.less_than: (xb) <- (xi64, xi64)
        less_than_0 = scale_0 < parameter_11
        ...
        # pd_op.assign_out_: (-1x501x256xf16) <- (-1x501x256xf16, -1x501x256xf16)
        ...
        cast_4 = paddle._C_ops.cast(add__8, paddle.float32)

        # pd_op.assign_out_: (-1x30xf32) <- (-1x30xf32, -1x30xf32)
        assign_out__4 = paddle._C_ops.assign_out_(cast_4, assign_value_1)

        # pd_op.cast: (-1x1x256xf32) <- (-1x1x256xf16)
        cast_5 = paddle._C_ops.cast(transpose_1, paddle.float32)

        # pd_op.assign_out_: (-1x1x256xf32) <- (-1x1x256xf32, -1x1x256xf32)
        assign_out__5 = paddle._C_ops.assign_out_(cast_5, assign_value_2)

        # pd_op.assign_out_: (xi64) <- (xi64, xi64)
        assign_out__6 = paddle._C_ops.assign_out_(scale_0, full_0)

        # pd_op.assign_out_: (xi64) <- (xi64, xi64)
        assign_out__7 = paddle._C_ops.assign_out_(slice_0, assign_value_3)

        # pd_op.assign_out_: (-1x256xf32) <- (-1x256xf32, -1x256xf32)
        assign_out__8 = paddle._C_ops.assign_out_(cast_2, assign_value_4)
    ...
    def builtin_module_597_0_0(self, constant_7, constant_6, constant_5, constant_8, parameter_177, parameter_178, constant_4, parameter_176, parameter_175, constant_3, constant_2, constant_1, constant_0, parameter_0, parameter_4, parameter_1, parameter_3, parameter_2, parameter_5, parameter_9, parameter_6, parameter_8, parameter_7, parameter_10, parameter_14, parameter_11, parameter_13, parameter_12, parameter_15, parameter_19, parameter_16, parameter_18, parameter_17, parameter_20, parameter_24, parameter_21, parameter_23, parameter_22, parameter_25, parameter_29, parameter_26, parameter_28, parameter_27, parameter_30, parameter_34, parameter_31, parameter_33, parameter_32, parameter_35, parameter_39, parameter_36, parameter_38, parameter_37, parameter_40, parameter_44, parameter_41, parameter_43, parameter_42, parameter_45, parameter_49, parameter_46, parameter_48, parameter_47, parameter_50, parameter_54, parameter_51, parameter_53, parameter_52, parameter_55, parameter_59, parameter_56, parameter_58, parameter_57, parameter_60, parameter_64, parameter_61, parameter_63, parameter_62, parameter_65, parameter_69, parameter_66, parameter_68, parameter_67, parameter_70, parameter_74, parameter_71, parameter_73, parameter_72, parameter_75, parameter_79, parameter_76, parameter_78, parameter_77, parameter_80, parameter_84, parameter_81, parameter_83, parameter_82, parameter_85, parameter_89, parameter_86, parameter_88, parameter_87, parameter_90, parameter_94, parameter_91, parameter_93, parameter_92, parameter_95, parameter_99, parameter_96, parameter_98, parameter_97, parameter_100, parameter_104, parameter_101, parameter_103, parameter_102, parameter_105, parameter_109, parameter_106, parameter_108, parameter_107, parameter_110, parameter_114, parameter_111, parameter_113, parameter_112, parameter_115, parameter_119, parameter_116, parameter_118, parameter_117, parameter_120, parameter_124, parameter_121, parameter_123, parameter_122, parameter_125, parameter_129, parameter_126, parameter_128, parameter_127, parameter_130, parameter_134, parameter_131, parameter_133, parameter_132, parameter_135, parameter_139, parameter_136, parameter_138, parameter_137, parameter_140, parameter_144, parameter_141, parameter_143, parameter_142, parameter_145, parameter_149, parameter_146, parameter_148, parameter_147, parameter_150, parameter_154, parameter_151, parameter_153, parameter_152, parameter_155, parameter_159, parameter_156, parameter_158, parameter_157, parameter_160, parameter_164, parameter_161, parameter_163, parameter_162, parameter_165, parameter_169, parameter_166, parameter_168, parameter_167, parameter_170, parameter_174, parameter_171, parameter_173, parameter_172, parameter_188, parameter_183, parameter_181, parameter_185, parameter_187, parameter_180, parameter_186, parameter_182, parameter_184, parameter_179, parameter_189, parameter_190, parameter_191, parameter_192, feed_0):
        ...
        # pd_op.hardswish: (-1x16x244x244xf16) <- (-1x16x244x244xf16)
        hardswish_0 = paddle._C_ops.hardswish(batch_norm__0)

        # pd_op.conv2d: (-1x16x244x244xf16) <- (-1x16x244x244xf16, 16x16x1x1xf16)
        ...
        # pd_op.relu_: (-1x16x244x244xf16) <- (-1x16x244x244xf16)
        relu__0 = paddle._C_ops.relu(batch_norm__6)

        # pd_op.depthwise_conv2d: (-1x16x122x122xf16) <- (-1x16x244x244xf16, 16x1x3x3xf16)
        depthwise_conv2d_0 = paddle._C_ops.depthwise_conv2d(relu__0, parameter_10, [2, 2], [1, 1], 'EXPLICIT', 16, [1, 1], 'NCHW')

        # pd_op.batch_norm_: (-1x16x122x122xf16, 16xf32, 16xf32, 16xf32, 16xf32, None) <- (-1x16x122x122xf16, 16xf32, 16xf32, 16xf32, 16xf32)
        ...
        # pd_op.relu_: (-1x16x122x122xf16) <- (-1x16x122x122xf16)
        relu__1 = paddle._C_ops.relu(batch_norm__12)

        # pd_op.conv2d: (-1x16x122x122xf16) <- (-1x16x122x122xf16, 16x16x1x1xf16)
        ...
        # pd_op.conv2d: (-1x72x122x122xf16) <- (-1x16x122x122xf16, 72x16x1x1xf16)
        conv2d_3 = paddle._C_ops.conv2d(batch_norm__18, parameter_20, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x72x122x122xf16, 72xf32, 72xf32, 72xf32, 72xf32, None) <- (-1x72x122x122xf16, 72xf32, 72xf32, 72xf32, 72xf32)
        ...
        # pd_op.relu_: (-1x72x122x122xf16) <- (-1x72x122x122xf16)
        relu__2 = paddle._C_ops.relu(batch_norm__24)

        # pd_op.depthwise_conv2d: (-1x72x61x61xf16) <- (-1x72x122x122xf16, 72x1x3x3xf16)
        depthwise_conv2d_1 = paddle._C_ops.depthwise_conv2d(relu__2, parameter_25, [2, 2], [1, 1], 'EXPLICIT', 72, [1, 1], 'NCHW')

        # pd_op.batch_norm_: (-1x72x61x61xf16, 72xf32, 72xf32, 72xf32, 72xf32, None) <- (-1x72x61x61xf16, 72xf32, 72xf32, 72xf32, 72xf32)
        ...
        # pd_op.relu_: (-1x72x61x61xf16) <- (-1x72x61x61xf16)
        relu__3 = paddle._C_ops.relu(batch_norm__30)

        # pd_op.conv2d: (-1x24x61x61xf16) <- (-1x72x61x61xf16, 24x72x1x1xf16)
        conv2d_4 = paddle._C_ops.conv2d(relu__3, parameter_30, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x24x61x61xf16, 24xf32, 24xf32, 24xf32, 24xf32, None) <- (-1x24x61x61xf16, 24xf32, 24xf32, 24xf32, 24xf32)
        ...
        # pd_op.conv2d: (-1x88x61x61xf16) <- (-1x24x61x61xf16, 88x24x1x1xf16)
        conv2d_5 = paddle._C_ops.conv2d(batch_norm__36, parameter_35, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x88x61x61xf16, 88xf32, 88xf32, 88xf32, 88xf32, None) <- (-1x88x61x61xf16, 88xf32, 88xf32, 88xf32, 88xf32)
        ...
        # pd_op.relu_: (-1x88x61x61xf16) <- (-1x88x61x61xf16)
        relu__4 = paddle._C_ops.relu(batch_norm__42)

        # pd_op.depthwise_conv2d: (-1x88x61x61xf16) <- (-1x88x61x61xf16, 88x1x3x3xf16)
        ...
        # pd_op.relu_: (-1x88x61x61xf16) <- (-1x88x61x61xf16)
        relu__5 = paddle._C_ops.relu(batch_norm__48)

        # pd_op.conv2d: (-1x24x61x61xf16) <- (-1x88x61x61xf16, 24x88x1x1xf16)
        ...
        # pd_op.add_: (-1x24x61x61xf16) <- (-1x24x61x61xf16, -1x24x61x61xf16)
        add__0 = paddle._C_ops.add(batch_norm__36, batch_norm__54)

        # pd_op.conv2d: (-1x96x61x61xf16) <- (-1x24x61x61xf16, 96x24x1x1xf16)
        conv2d_7 = paddle._C_ops.conv2d(add__0, parameter_50, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x96x61x61xf16, 96xf32, 96xf32, 96xf32, 96xf32, None) <- (-1x96x61x61xf16, 96xf32, 96xf32, 96xf32, 96xf32)
        ...
        # pd_op.hardswish: (-1x96x61x61xf16) <- (-1x96x61x61xf16)
        hardswish_1 = paddle._C_ops.hardswish(batch_norm__60)

        # pd_op.depthwise_conv2d: (-1x96x31x31xf16) <- (-1x96x61x61xf16, 96x1x5x5xf16)
        depthwise_conv2d_3 = paddle._C_ops.depthwise_conv2d(hardswish_1, parameter_55, [2, 2], [2, 2], 'EXPLICIT', 96, [1, 1], 'NCHW')

        # pd_op.batch_norm_: (-1x96x31x31xf16, 96xf32, 96xf32, 96xf32, 96xf32, None) <- (-1x96x31x31xf16, 96xf32, 96xf32, 96xf32, 96xf32)
        ...
        # pd_op.hardswish: (-1x96x31x31xf16) <- (-1x96x31x31xf16)
        hardswish_2 = paddle._C_ops.hardswish(batch_norm__66)

        # pd_op.conv2d: (-1x40x31x31xf16) <- (-1x96x31x31xf16, 40x96x1x1xf16)
        conv2d_8 = paddle._C_ops.conv2d(hardswish_2, parameter_60, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x40x31x31xf16, 40xf32, 40xf32, 40xf32, 40xf32, None) <- (-1x40x31x31xf16, 40xf32, 40xf32, 40xf32, 40xf32)
        ...
        # pd_op.conv2d: (-1x240x31x31xf16) <- (-1x40x31x31xf16, 240x40x1x1xf16)
        conv2d_9 = paddle._C_ops.conv2d(batch_norm__72, parameter_65, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x240x31x31xf16, 240xf32, 240xf32, 240xf32, 240xf32, None) <- (-1x240x31x31xf16, 240xf32, 240xf32, 240xf32, 240xf32)
        ...
        # pd_op.hardswish: (-1x240x31x31xf16) <- (-1x240x31x31xf16)
        hardswish_3 = paddle._C_ops.hardswish(batch_norm__78)

        # pd_op.depthwise_conv2d: (-1x240x31x31xf16) <- (-1x240x31x31xf16, 240x1x5x5xf16)
        ...
        # pd_op.conv2d: (-1x40x31x31xf16) <- (-1x240x31x31xf16, 40x240x1x1xf16)
        ...
        # pd_op.add_: (-1x40x31x31xf16) <- (-1x40x31x31xf16, -1x40x31x31xf16)
        add__1 = paddle._C_ops.add(batch_norm__72, batch_norm__90)
        ...
        # pd_op.add_: (-1x40x31x31xf16) <- (-1x40x31x31xf16, -1x40x31x31xf16)
        add__2 = paddle._C_ops.add(add__1, batch_norm__108)

        # pd_op.conv2d: (-1x120x31x31xf16) <- (-1x40x31x31xf16, 120x40x1x1xf16)
        conv2d_13 = paddle._C_ops.conv2d(add__2, parameter_95, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x120x31x31xf16, 120xf32, 120xf32, 120xf32, 120xf32, None) <- (-1x120x31x31xf16, 120xf32, 120xf32, 120xf32, 120xf32)
        ...
        # pd_op.hardswish: (-1x120x31x31xf16) <- (-1x120x31x31xf16)
        hardswish_7 = paddle._C_ops.hardswish(batch_norm__114)

        # pd_op.depthwise_conv2d: (-1x120x31x31xf16) <- (-1x120x31x31xf16, 120x1x5x5xf16)
        ...
        # pd_op.conv2d: (-1x48x31x31xf16) <- (-1x120x31x31xf16, 48x120x1x1xf16)
        conv2d_14 = paddle._C_ops.conv2d(hardswish_8, parameter_105, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x48x31x31xf16, 48xf32, 48xf32, 48xf32, 48xf32, None) <- (-1x48x31x31xf16, 48xf32, 48xf32, 48xf32, 48xf32)
        ...
        # pd_op.conv2d: (-1x144x31x31xf16) <- (-1x48x31x31xf16, 144x48x1x1xf16)
        conv2d_15 = paddle._C_ops.conv2d(batch_norm__126, parameter_110, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x144x31x31xf16, 144xf32, 144xf32, 144xf32, 144xf32, None) <- (-1x144x31x31xf16, 144xf32, 144xf32, 144xf32, 144xf32)
        ...
        # pd_op.hardswish: (-1x144x31x31xf16) <- (-1x144x31x31xf16)
        hardswish_9 = paddle._C_ops.hardswish(batch_norm__132)

        # pd_op.depthwise_conv2d: (-1x144x31x31xf16) <- (-1x144x31x31xf16, 144x1x5x5xf16)
        ...
        # pd_op.conv2d: (-1x48x31x31xf16) <- (-1x144x31x31xf16, 48x144x1x1xf16)
        ...
        # pd_op.add_: (-1x48x31x31xf16) <- (-1x48x31x31xf16, -1x48x31x31xf16)
        add__3 = paddle._C_ops.add(batch_norm__126, batch_norm__144)

        # pd_op.conv2d: (-1x288x31x31xf16) <- (-1x48x31x31xf16, 288x48x1x1xf16)
        conv2d_17 = paddle._C_ops.conv2d(add__3, parameter_125, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x288x31x31xf16, 288xf32, 288xf32, 288xf32, 288xf32, None) <- (-1x288x31x31xf16, 288xf32, 288xf32, 288xf32, 288xf32)
        ...
        # pd_op.hardswish: (-1x288x31x31xf16) <- (-1x288x31x31xf16)
        hardswish_11 = paddle._C_ops.hardswish(batch_norm__150)

        # pd_op.depthwise_conv2d: (-1x288x16x16xf16) <- (-1x288x31x31xf16, 288x1x5x5xf16)
        depthwise_conv2d_8 = paddle._C_ops.depthwise_conv2d(hardswish_11, parameter_130, [2, 2], [2, 2], 'EXPLICIT', 288, [1, 1], 'NCHW')

        # pd_op.batch_norm_: (-1x288x16x16xf16, 288xf32, 288xf32, 288xf32, 288xf32, None) <- (-1x288x16x16xf16, 288xf32, 288xf32, 288xf32, 288xf32)
        ...
        # pd_op.hardswish: (-1x288x16x16xf16) <- (-1x288x16x16xf16)
        hardswish_12 = paddle._C_ops.hardswish(batch_norm__156)

        # pd_op.conv2d: (-1x96x16x16xf16) <- (-1x288x16x16xf16, 96x288x1x1xf16)
        conv2d_18 = paddle._C_ops.conv2d(hardswish_12, parameter_135, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x96x16x16xf16, 96xf32, 96xf32, 96xf32, 96xf32, None) <- (-1x96x16x16xf16, 96xf32, 96xf32, 96xf32, 96xf32)
        ...
        # pd_op.conv2d: (-1x576x16x16xf16) <- (-1x96x16x16xf16, 576x96x1x1xf16)
        conv2d_19 = paddle._C_ops.conv2d(batch_norm__162, parameter_140, [1, 1], [0, 0], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.batch_norm_: (-1x576x16x16xf16, 576xf32, 576xf32, 576xf32, 576xf32, None) <- (-1x576x16x16xf16, 576xf32, 576xf32, 576xf32, 576xf32)
        ...
        # pd_op.hardswish: (-1x576x16x16xf16) <- (-1x576x16x16xf16)
        hardswish_13 = paddle._C_ops.hardswish(batch_norm__168)

        # pd_op.depthwise_conv2d: (-1x576x16x16xf16) <- (-1x576x16x16xf16, 576x1x5x5xf16)
        ...
        # pd_op.conv2d: (-1x96x16x16xf16) <- (-1x576x16x16xf16, 96x576x1x1xf16)
        ...
        # pd_op.add_: (-1x96x16x16xf16) <- (-1x96x16x16xf16, -1x96x16x16xf16)
        add__4 = paddle._C_ops.add(batch_norm__162, batch_norm__180)
        ...
        # pd_op.add_: (-1x96x16x16xf16) <- (-1x96x16x16xf16, -1x96x16x16xf16)
        add__5 = paddle._C_ops.add(add__4, batch_norm__198)
        ...
        # pd_op.shape: (4xi32) <- (-1x576x16x16xf16)
        shape_0 = paddle._C_ops.shape(paddle.cast(hardswish_17, 'float32'))
        ...
        combine_0 = [slice_0, constant_2, constant_3]

        # pd_op.reshape_: (-1x576x256xf16, 0x-1x576x16x16xf16) <- (-1x576x16x16xf16, [xi32, 1xi32, 1xi32])
        reshape__0, reshape__1 = (lambda x, f: f(x))(paddle._C_ops.reshape(hardswish_17, combine_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.transpose: (-1x256x576xf16) <- (-1x576x256xf16)
        transpose_0 = paddle._C_ops.transpose(reshape__0, [0, 2, 1])

        # pd_op.shape: (3xi32) <- (-1x256x576xf16)
        ...
        combine_1 = [slice_1, parameter_175]

        # pd_op.memcpy_h2d: (xi32) <- (xi32)
        memcpy_h2d_0 = paddle._C_ops.memcpy_h2d(slice_1, 1)

        # builtin.combine: ([xi32, xi32]) <- (xi32, xi32)
        combine_2 = [memcpy_h2d_0, parameter_175]
        ...
        combine_3 = [slice_1, parameter_176, parameter_175]

        # pd_op.memcpy_h2d: (xi32) <- (xi32)
        memcpy_h2d_1 = paddle._C_ops.memcpy_h2d(slice_1, 1)

        # builtin.combine: ([xi32, xi32, xi32]) <- (xi32, xi32, xi32)
        combine_4 = [memcpy_h2d_1, parameter_176, parameter_175]

        # pd_op.stack: (3xi32) <- ([xi32, xi32, xi32])
        stack_1 = paddle._C_ops.stack(combine_4, 0)

        # pd_op.full_with_tensor: (-1x501x256xf16) <- (1xf32, 3xi32)
        ...
        full_with_tensor_2 = paddle._C_ops.full_with_tensor(full_0, stack_2, paddle.int32)
        ...
        # pd_op.full: (xi64) <- ()
        full_1 = paddle._C_ops.full([], float('0'), paddle.int64, paddle.framework._current_expected_place())

        # pd_op.assign_value: (-1x256xf32) <- ()
        assign_value_2 = paddle.to_tensor([float('1.77113e+27')], dtype=paddle.float64).reshape([1])

        # pd_op.less_than: (xb) <- (xi64, xi64)
        less_than_0 = full_1 < parameter_177

        # pd_op.full: (-1x501x256xf16) <- ()
        full_2 = paddle._C_ops.full([], float('0'), paddle.float16, paddle.framework._current_expected_place())
        ...
        # pd_op.while: (-1xi32, -1x30xf32, -1x256xf16, -1x501x256xf16, -1x501x256xf16, xi64, -1x1x256xf32, -1x30xf32, -1x256xf32, xi64) <- (xb, -1xi32, -1x30xf32, -1x256xf16, -1x501x256xf16, -1x501x256xf16, xi64, -1x1x256xf32, -1x30xf32, -1x256xf32, xi64)
        import os
        ...
        while_loop_counter_932 = 0
        while less_than_0:
            less_than_0, full_with_tensor_2, assign_value_0, full_with_tensor_0, full_2, full_with_tensor_1, full_1, assign_value_1, assign_value_3, assign_value_2, assign_value_4, = self.pd_op_while_932_0_0(constant_4, parameter_178, constant_5, transpose_0, parameter_179, parameter_180, parameter_181, constant_1, parameter_182, constant_6, parameter_183, parameter_184, parameter_185, parameter_186, parameter_187, parameter_188, constant_7, parameter_177, less_than_0, full_with_tensor_2, assign_value_0, full_with_tensor_0, full_2, full_with_tensor_1, full_1, assign_value_1, assign_value_3, assign_value_2, assign_value_4)
            while_loop_counter_932 += 1
            if while_loop_counter_932 > kWhileLoopLimit:
                break
            
        while_0, while_1, while_2, while_3, while_4, while_5, while_6, while_7, while_8, while_9, = full_with_tensor_2, assign_value_0, full_with_tensor_0, full_2, full_with_tensor_1, full_1, assign_value_1, assign_value_3, assign_value_2, assign_value_4,

        # pd_op.matmul: (-1x501x30xf16) <- (-1x501x256xf16, 256x30xf16)
        matmul_0 = paddle.matmul(while_3, parameter_187, transpose_x=False, transpose_y=False)

        # pd_op.add_: (-1x501x30xf16) <- (-1x501x30xf16, 30xf16)
        add__6 = paddle._C_ops.add(matmul_0, parameter_188)

        # pd_op.softmax_: (-1x501x30xf16) <- (-1x501x30xf16)
        softmax__0 = paddle._C_ops.softmax(add__6, -1)

        # pd_op.transpose: (-1x576x256xf16) <- (-1x256x576xf16)
        transpose_1 = paddle._C_ops.transpose(transpose_0, [0, 2, 1])

        # pd_op.matmul: (-1x576x501xf16) <- (-1x576x256xf16, 256x501xf16)
        matmul_1 = paddle.matmul(transpose_1, parameter_189, transpose_x=False, transpose_y=False)

        # pd_op.add_: (-1x576x501xf16) <- (-1x576x501xf16, 501xf16)
        add__7 = paddle._C_ops.add(matmul_1, parameter_190)

        # pd_op.transpose: (-1x501x576xf16) <- (-1x576x501xf16)
        transpose_2 = paddle._C_ops.transpose(add__7, [0, 2, 1])

        # builtin.combine: ([-1x501x256xf16, -1x501x576xf16]) <- (-1x501x256xf16, -1x501x576xf16)
        combine_6 = [while_3, transpose_2]

        # pd_op.concat: (-1x501x832xf16) <- ([-1x501x256xf16, -1x501x576xf16], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_6, constant_8)

        # pd_op.matmul: (-1x501x4xf16) <- (-1x501x832xf16, 832x4xf16)
        matmul_2 = paddle.matmul(concat_0, parameter_191, transpose_x=False, transpose_y=False)

        # pd_op.add_: (-1x501x4xf16) <- (-1x501x4xf16, 4xf16)
        add__8 = paddle._C_ops.add(matmul_2, parameter_192)

        # pd_op.sigmoid_: (-1x501x4xf16) <- (-1x501x4xf16)
        sigmoid__0 = paddle._C_ops.sigmoid(add__8)

        # pd_op.scale_: (-1x501x4xf16) <- (-1x501x4xf16, 1xf32)
        scale__0 = paddle._C_ops.scale(sigmoid__0, constant_4, float('0'), True)

        # pd_op.scale_: (-1x501x30xf16) <- (-1x501x30xf16, 1xf32)
        scale__1 = paddle._C_ops.scale(softmax__0, constant_4, float('0'), True)
...
class ModuleOp(paddle.nn.Layer, BlockEntries):
    ...
    def forward(self, constant_7, constant_6, constant_5, constant_8, parameter_177, parameter_178, constant_4, parameter_176, parameter_175, constant_3, constant_2, constant_1, constant_0, parameter_0, parameter_4, parameter_1, parameter_3, parameter_2, parameter_5, parameter_9, parameter_6, parameter_8, parameter_7, parameter_10, parameter_14, parameter_11, parameter_13, parameter_12, parameter_15, parameter_19, parameter_16, parameter_18, parameter_17, parameter_20, parameter_24, parameter_21, parameter_23, parameter_22, parameter_25, parameter_29, parameter_26, parameter_28, parameter_27, parameter_30, parameter_34, parameter_31, parameter_33, parameter_32, parameter_35, parameter_39, parameter_36, parameter_38, parameter_37, parameter_40, parameter_44, parameter_41, parameter_43, parameter_42, parameter_45, parameter_49, parameter_46, parameter_48, parameter_47, parameter_50, parameter_54, parameter_51, parameter_53, parameter_52, parameter_55, parameter_59, parameter_56, parameter_58, parameter_57, parameter_60, parameter_64, parameter_61, parameter_63, parameter_62, parameter_65, parameter_69, parameter_66, parameter_68, parameter_67, parameter_70, parameter_74, parameter_71, parameter_73, parameter_72, parameter_75, parameter_79, parameter_76, parameter_78, parameter_77, parameter_80, parameter_84, parameter_81, parameter_83, parameter_82, parameter_85, parameter_89, parameter_86, parameter_88, parameter_87, parameter_90, parameter_94, parameter_91, parameter_93, parameter_92, parameter_95, parameter_99, parameter_96, parameter_98, parameter_97, parameter_100, parameter_104, parameter_101, parameter_103, parameter_102, parameter_105, parameter_109, parameter_106, parameter_108, parameter_107, parameter_110, parameter_114, parameter_111, parameter_113, parameter_112, parameter_115, parameter_119, parameter_116, parameter_118, parameter_117, parameter_120, parameter_124, parameter_121, parameter_123, parameter_122, parameter_125, parameter_129, parameter_126, parameter_128, parameter_127, parameter_130, parameter_134, parameter_131, parameter_133, parameter_132, parameter_135, parameter_139, parameter_136, parameter_138, parameter_137, parameter_140, parameter_144, parameter_141, parameter_143, parameter_142, parameter_145, parameter_149, parameter_146, parameter_148, parameter_147, parameter_150, parameter_154, parameter_151, parameter_153, parameter_152, parameter_155, parameter_159, parameter_156, parameter_158, parameter_157, parameter_160, parameter_164, parameter_161, parameter_163, parameter_162, parameter_165, parameter_169, parameter_166, parameter_168, parameter_167, parameter_170, parameter_174, parameter_171, parameter_173, parameter_172, parameter_188, parameter_183, parameter_181, parameter_185, parameter_187, parameter_180, parameter_186, parameter_182, parameter_184, parameter_179, parameter_189, parameter_190, parameter_191, parameter_192, feed_0):
        return self.builtin_module_597_0_0(constant_7, constant_6, constant_5, constant_8, parameter_177, parameter_178, constant_4, parameter_176, parameter_175, constant_3, constant_2, constant_1, constant_0, parameter_0, parameter_4, parameter_1, parameter_3, parameter_2, parameter_5, parameter_9, parameter_6, parameter_8, parameter_7, parameter_10, parameter_14, parameter_11, parameter_13, parameter_12, parameter_15, parameter_19, parameter_16, parameter_18, parameter_17, parameter_20, parameter_24, parameter_21, parameter_23, parameter_22, parameter_25, parameter_29, parameter_26, parameter_28, parameter_27, parameter_30, parameter_34, parameter_31, parameter_33, parameter_32, parameter_35, parameter_39, parameter_36, parameter_38, parameter_37, parameter_40, parameter_44, parameter_41, parameter_43, parameter_42, parameter_45, parameter_49, parameter_46, parameter_48, parameter_47, parameter_50, parameter_54, parameter_51, parameter_53, parameter_52, parameter_55, parameter_59, parameter_56, parameter_58, parameter_57, parameter_60, parameter_64, parameter_61, parameter_63, parameter_62, parameter_65, parameter_69, parameter_66, parameter_68, parameter_67, parameter_70, parameter_74, parameter_71, parameter_73, parameter_72, parameter_75, parameter_79, parameter_76, parameter_78, parameter_77, parameter_80, parameter_84, parameter_81, parameter_83, parameter_82, parameter_85, parameter_89, parameter_86, parameter_88, parameter_87, parameter_90, parameter_94, parameter_91, parameter_93, parameter_92, parameter_95, parameter_99, parameter_96, parameter_98, parameter_97, parameter_100, parameter_104, parameter_101, parameter_103, parameter_102, parameter_105, parameter_109, parameter_106, parameter_108, parameter_107, parameter_110, parameter_114, parameter_111, parameter_113, parameter_112, parameter_115, parameter_119, parameter_116, parameter_118, parameter_117, parameter_120, parameter_124, parameter_121, parameter_123, parameter_122, parameter_125, parameter_129, parameter_126, parameter_128, parameter_127, parameter_130, parameter_134, parameter_131, parameter_133, parameter_132, parameter_135, parameter_139, parameter_136, parameter_138, parameter_137, parameter_140, parameter_144, parameter_141, parameter_143, parameter_142, parameter_145, parameter_149, parameter_146, parameter_148, parameter_147, parameter_150, parameter_154, parameter_151, parameter_153, parameter_152, parameter_155, parameter_159, parameter_156, parameter_158, parameter_157, parameter_160, parameter_164, parameter_161, parameter_163, parameter_162, parameter_165, parameter_169, parameter_166, parameter_168, parameter_167, parameter_170, parameter_174, parameter_171, parameter_173, parameter_172, parameter_188, parameter_183, parameter_181, parameter_185, parameter_187, parameter_180, parameter_186, parameter_182, parameter_184, parameter_179, parameter_189, parameter_190, parameter_191, parameter_192, feed_0)

@unittest.skipIf(need_skip, skip_message)
class Test_builtin_module_597_0_0(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            ...
            paddle.uniform([16], dtype='float32', min=0, max=0.5),
            # parameter_20
            paddle.uniform([72, 16, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_24
            ...
            paddle.uniform([88], dtype='float32', min=0, max=0.5),
            # parameter_45
            paddle.uniform([24, 88, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_49
            paddle.uniform([24], dtype='float32', min=0, max=0.5),
            # parameter_46
            paddle.uniform([24], dtype='float32', min=0, max=0.5),
            ...
            paddle.uniform([120], dtype='float32', min=0, max=0.5),
            # parameter_105
            paddle.uniform([48, 120, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_109
            ...
            paddle.uniform([48], dtype='float32', min=0, max=0.5),
            # parameter_110
            paddle.uniform([144, 48, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_114
            ...
            paddle.uniform([144], dtype='float32', min=0, max=0.5),
            # parameter_120
            paddle.uniform([48, 144, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_124
            paddle.uniform([48], dtype='float32', min=0, max=0.5),
            # parameter_121
            paddle.uniform([48], dtype='float32', min=0, max=0.5),
            ...
            paddle.uniform([48], dtype='float32', min=0, max=0.5),
            # parameter_125
            paddle.uniform([288, 48, 1, 1], dtype='float16', min=0, max=0.5),
            # parameter_129
