...
class BlockEntries:
    ...
    def builtin_module_544_0_0(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.arange: (-1xi64) <- (1xi64, xi64, 1xi64)
        arange_6 = paddle.arange(full_0, cast_18, full_1, dtype='int64')

        # pd_op.cast: (-1xf32) <- (-1xi64)
        cast_19 = paddle._C_ops.cast(arange_6, paddle.float32)

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_12 = paddle._C_ops.scale(cast_19, full_2, float('0.5'), True)

        # pd_op.full: (1xf32) <- ()
        full_7 = paddle._C_ops.full([1], float('64'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_13 = paddle._C_ops.scale(scale_12, full_7, float('0'), True)

        # pd_op.cast: (xi64) <- (xi32)
        cast_20 = paddle._C_ops.cast(slice_14, paddle.int64)

        # pd_op.arange: (-1xi64) <- (1xi64, xi64, 1xi64)
        arange_7 = paddle.arange(full_0, cast_20, full_1, dtype='int64')

        # pd_op.cast: (-1xf32) <- (-1xi64)
        cast_21 = paddle._C_ops.cast(arange_7, paddle.float32)

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_14 = paddle._C_ops.scale(cast_21, full_2, float('0.5'), True)
        ...
        # pd_op.reshape: (1x-1x2xf32, 0x-1x-1x2xi64) <- (-1x-1x2xf32, 3xi64)
        reshape_6, reshape_7 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_22, full_int_array_5), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        # pd_op.arange: (-1xi64) <- (1xi64, xi64, 1xi64)
        arange_8 = paddle.arange(full_0, cast_24, full_1, dtype='int64')

        # pd_op.cast: (-1xf32) <- (-1xi64)
        cast_25 = paddle._C_ops.cast(arange_8, paddle.float32)

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_16 = paddle._C_ops.scale(cast_25, full_2, float('0.5'), True)

        # pd_op.full: (1xf32) <- ()
        full_8 = paddle._C_ops.full([1], float('128'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_17 = paddle._C_ops.scale(scale_16, full_8, float('0'), True)

        # pd_op.cast: (xi64) <- (xi32)
        cast_26 = paddle._C_ops.cast(slice_18, paddle.int64)

        # pd_op.arange: (-1xi64) <- (1xi64, xi64, 1xi64)
        arange_9 = paddle.arange(full_0, cast_26, full_1, dtype='int64')

        # pd_op.cast: (-1xf32) <- (-1xi64)
        cast_27 = paddle._C_ops.cast(arange_9, paddle.float32)

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_18 = paddle._C_ops.scale(cast_27, full_2, float('0.5'), True)

        # pd_op.scale: (-1xf32) <- (-1xf32, 1xf32)
        scale_19 = paddle._C_ops.scale(scale_18, full_8, float('0'), True)
        ...
        # pd_op.reshape: (1x-1x2xf32, 0x-1x-1x2xi64) <- (-1x-1x2xf32, 3xi64)
        reshape_8, reshape_9 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_28, full_int_array_5), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
...
class Block_builtin_module_544_0_0(paddle.nn.Layer, BlockEntries):
    def __init__(self):
        super().__init__()
...
class Test_builtin_module_544_0_0(CinnTestBase, unittest.TestCase):
    ...
    def entry(self, use_cinn):
        net = Block_builtin_module_544_0_0()
