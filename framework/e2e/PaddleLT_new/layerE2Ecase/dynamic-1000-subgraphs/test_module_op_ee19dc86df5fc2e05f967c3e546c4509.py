...
class BlockEntries:
    ...
    def builtin_module_82_0_0(self, data_0, data_1, data_2):
        ...
        unsqueeze_0, unsqueeze_1 = (lambda x, f: f(x))(paddle._C_ops.unsqueeze(data_1, full_int_array_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.full: (1x500x1xi64) <- ()
        full_0 = paddle._C_ops.full([1, 500, 1], float('0'), paddle.int64, paddle.framework._current_expected_place())
        ...
        unsqueeze_2, unsqueeze_3 = (lambda x, f: f(x))(paddle._C_ops.unsqueeze(data_2, full_int_array_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.expand_as: (1x500x128xi32) <- (-1x-1x1xi32, None)
        expand_as_0 = paddle._C_ops.expand_as(unsqueeze_2, None, [1, 500, 128])

        # pd_op.full: (xi32) <- ()
        full_3 = paddle._C_ops.full([], float('0'), paddle.int32, paddle.framework._current_expected_place())
        ...
        reshape_2, reshape_3 = (lambda x, f: f(x))(paddle._C_ops.reshape(masked_select_0, full_int_array_2), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
