...
class BlockEntries:
    ...
    def builtin_module_101_0_0(self, parameter_0, parameter_1, data_0, data_1, data_2, data_3):
        ...
        # pd_op.full: (1xi32) <- ()
        full_0 = paddle._C_ops.full([1], float('80'), paddle.int32, paddle.core.CPUPlace())

        # pd_op.one_hot: (-1x80xf32) <- (-1xi32, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(data_1 % paddle.cast(full_0, data_1.dtype), full_0)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [2, 3]

        # pd_op.unsqueeze: (-1x80x1x1xf32, 0x-1x80xf32) <- (-1x80xf32, 2xi64)
        unsqueeze_0, unsqueeze_1 = (lambda x, f: f(x))(paddle._C_ops.unsqueeze(one_hot_0, full_int_array_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        full_int_array_2 = [6, 28, 28]

        # pd_op.reshape: (6x28x28xf32, 0x-1xi64) <- (-1xf32, 3xi64)
        reshape_2, reshape_3 = (lambda x, f: f(x))(paddle._C_ops.reshape(gather_nd_0, full_int_array_2), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        unsqueeze_2, unsqueeze_3 = (lambda x, f: f(x))(paddle._C_ops.unsqueeze(data_3, full_int_array_3), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.full: (1xf32) <- ()
        full_1 = paddle._C_ops.full([1], float('1'), paddle.float32, paddle.framework._current_expected_place())

        # pd_op.sigmoid_cross_entropy_with_logits: (6x28x28xf32) <- (6x28x28xf32, -1x-1x-1xf32, None)
        sigmoid_cross_entropy_with_logits_0 = paddle._C_ops.sigmoid_cross_entropy_with_logits(reshape_2, cast_0, None, False, -100)

        # pd_op.multiply: (6x28x28xf32) <- (6x28x28xf32, -1x1x1xf32)
        multiply_0 = sigmoid_cross_entropy_with_logits_0 * unsqueeze_2

        # pd_op.mean_all: (xf32) <- (6x28x28xf32)
...
class Block_builtin_module_101_0_0(paddle.nn.Layer, BlockEntries):
    def __init__(self):
        super().__init__()

    def forward(self, parameter_0, parameter_1, data_0, data_1, data_2, data_3):
        args = [parameter_0, parameter_1, data_0, data_1, data_2, data_3]
    ...
    def op_conv2d_0(self, parameter_0, parameter_1, data_0, data_1, data_2, data_3):
...
class Test_builtin_module_101_0_0(CinnTestBase, unittest.TestCase):
    ...
    def entry(self, use_cinn):
        net = Block_builtin_module_101_0_0()
