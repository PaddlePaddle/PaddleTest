...
class BlockEntries:
    ...
    def builtin_module_103_0_0(self, data_3, data_0, data_1, data_2):

        # pd_op.full: (xi32) <- ()
        full_0 = paddle._C_ops.full([], float('7'), paddle.int32, paddle.framework._current_expected_place())
        ...
        reshape_0, reshape_1 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_0, full_int_array_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.cast: (1xi64) <- (1xi32)
        cast_1 = paddle._C_ops.cast(floor_divide_1, paddle.int64)

        # pd_op.reshape: (xi64, 0x1xi64) <- (1xi64, 0xi64)
        reshape_2, reshape_3 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_1, full_int_array_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.full: (xi64) <- ()
        full_2 = paddle._C_ops.full([], float('7'), paddle.int64, paddle.core.CPUPlace())

        # pd_op.cast: (1xi64) <- (1xi32)
        cast_2 = paddle._C_ops.cast(data_2, paddle.int64)

        # pd_op.reshape: (xi64, 0x1xi64) <- (1xi64, 0xi64)
        reshape_4, reshape_5 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_2, full_int_array_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        reshape_6, reshape_7 = (lambda x, f: f(x))(paddle._C_ops.reshape(data_3, stack_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.transpose: (-1x-1x7x-1x7x-1xf32) <- (-1x-1x-1x7x7x-1xf32)
        transpose_0 = paddle._C_ops.transpose(reshape_6, [0, 1, 3, 2, 4, 5])

        # pd_op.cast: (1xi64) <- (1xi32)
        cast_3 = paddle._C_ops.cast(data_0, paddle.int64)

        # pd_op.reshape: (xi64, 0x1xi64) <- (1xi64, 0xi64)
        reshape_8, reshape_9 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_3, full_int_array_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.cast: (1xi64) <- (1xi32)
        cast_4 = paddle._C_ops.cast(data_1, paddle.int64)

        # pd_op.reshape: (xi64, 0x1xi64) <- (1xi64, 0xi64)
        reshape_10, reshape_11 = (lambda x, f: f(x))(paddle._C_ops.reshape(cast_4, full_int_array_0), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        reshape_12, reshape_13 = (lambda x, f: f(x))(paddle._C_ops.reshape(transpose_0, stack_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
...
class Block_builtin_module_103_0_0(paddle.nn.Layer, BlockEntries):
    def __init__(self):
        super().__init__()
...
class Test_builtin_module_103_0_0(CinnTestBase, unittest.TestCase):
    ...
    def entry(self, use_cinn):
        net = Block_builtin_module_103_0_0()
