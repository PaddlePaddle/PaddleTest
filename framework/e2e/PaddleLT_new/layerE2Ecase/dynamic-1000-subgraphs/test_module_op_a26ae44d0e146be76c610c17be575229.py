...
class BlockEntries:
    ...
    def builtin_module_97_0_0(self, parameter_0, parameter_1, data_0, data_1):

        # pd_op.conv2d: (-1x76x-1x-1xf32) <- (-1x384x-1x-1xf32, 76x384x3x3xf32)
        conv2d_0 = paddle._C_ops.conv2d(data_0, parameter_0, [1, 1], [1, 1], 'EXPLICIT', [1, 1], 1, 'NCHW')

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_0 = [1, -1, 1, 1]

        # pd_op.reshape: (1x76x1x1xf32, 0x76xi64) <- (76xf32, 4xi64)
        ...
        # pd_op.add: (-1x76x-1x-1xf32) <- (-1x76x-1x-1xf32, 1x76x1x1xf32)
        ...
        # pd_op.flatten: (-1x76x-1xf32, 0x-1x76x-1x-1xf32) <- (-1x76x-1x-1xf32)
        flatten_2, flatten_3 = (lambda x, f: f(x))(paddle._C_ops.flatten(add_0, 2, 3), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.transpose: (-1x-1x76xf32) <- (-1x76x-1xf32)
