...
class BlockEntries:
    ...
    def builtin_module_94_0_0(self, data_0, data_1, data_2, data_3, data_4):
        ...
        huber_loss_0, huber_loss_1 = (lambda x, f: f(x))(paddle._C_ops.huber_loss(masked_select_0, masked_select_1, float('1')), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
        ...
        cross_entropy_with_softmax_0, cross_entropy_with_softmax_1 = (lambda x, f: f(x))(paddle._C_ops.cross_entropy_with_softmax(data_3, data_4, False, True, True, -100, -1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_1 = [-1]

        # pd_op.squeeze: (-1x-1xf32, 0x-1x-1x1xf32) <- (-1x-1x1xf32, 1xi64)
        squeeze_0, squeeze_1 = (lambda x, f: f(x))(paddle._C_ops.squeeze(cross_entropy_with_softmax_1, full_int_array_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))

        # pd_op.squeeze: (-1x-1xi64, 0x-1x-1x1xi64) <- (-1x-1x1xi64, 1xi64)
        squeeze_2, squeeze_3 = (lambda x, f: f(x))(paddle._C_ops.squeeze(data_4, full_int_array_1), lambda out: out if isinstance(out, (list, tuple)) else (out, None))
