...
class BlockEntries:
    def builtin_module_502_0_0(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6, data_7, data_8):
        ...
        # pd_op.full: (1xf32) <- ()
        full_2 = paddle._C_ops.full([1], float('96'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.full: (1xf32) <- ()
        full_3 = paddle._C_ops.full([1], float('1'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.arange: (96xi64) <- (1xf32, 1xf32, 1xf32)
        ...
        # pd_op.reshape: (9216x2xf32, 0x96x96x2xi64) <- (96x96x2xf32, 2xi64)
        ...
        full_5 = paddle._C_ops.full([9216, 1], float('8'), paddle.float32, paddle.framework._current_expected_place())

        # pd_op.full: (1xf32) <- ()
        full_6 = paddle._C_ops.full([1], float('48'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.arange: (48xi64) <- (1xf32, 1xf32, 1xf32)
        ...
        # pd_op.reshape: (2304x2xf32, 0x48x48x2xi64) <- (48x48x2xf32, 2xi64)
        ...
        full_8 = paddle._C_ops.full([2304, 1], float('16'), paddle.float32, paddle.framework._current_expected_place())

        # pd_op.full: (1xf32) <- ()
        full_9 = paddle._C_ops.full([1], float('24'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.arange: (24xi64) <- (1xf32, 1xf32, 1xf32)
        ...
        # pd_op.reshape: (576x2xf32, 0x24x24x2xi64) <- (24x24x2xf32, 2xi64)
        ...
        full_11 = paddle._C_ops.full([576, 1], float('32'), paddle.float32, paddle.framework._current_expected_place())

        # pd_op.full: (1xi32) <- ()
        full_12 = paddle._C_ops.full([1], float('0'), paddle.int32, paddle.core.CPUPlace())

        # builtin.combine: ([9216x2xf32, 2304x2xf32, 576x2xf32]) <- (9216x2xf32, 2304x2xf32, 576x2xf32)
        combine_9 = [reshape_0, reshape_2, reshape_4]

        # pd_op.concat: (12096x2xf32) <- ([9216x2xf32, 2304x2xf32, 576x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_9, full_12)

        # pd_op.cast: (12096x2xf32) <- (12096x2xf32)
        cast_3 = paddle._C_ops.cast(concat_3, paddle.float32)

        # builtin.combine: ([9216x1xf32, 2304x1xf32, 576x1xf32]) <- (9216x1xf32, 2304x1xf32, 576x1xf32)
        combine_10 = [full_5, full_8, full_11]

        # pd_op.concat: (12096x1xf32) <- ([9216x1xf32, 2304x1xf32, 576x1xf32], 1xi32)
        concat_4 = paddle._C_ops.concat(combine_10, full_12)

        # pd_op.assign: (12096x1xf32) <- (12096x1xf32)
        assign_2 = concat_4
        ...
        # pd_op.divide: (12096x2xf32) <- (12096x2xf32, 12096x1xf32)
        divide_0 = cast_3 / concat_4

        # pd_op.add: (-1x12096x2xf32) <- (-1x-1x-1xf32, 12096x2xf32)
        ...
        # pd_op.subtract: (-1x12096x2xf32) <- (-1x12096x2xf32, -1x-1x-1xf32)
        subtract_0 = add_0 - scale_6

        # pd_op.add: (-1x12096x2xf32) <- (-1x12096x2xf32, -1x-1x-1xf32)
        add_1 = add_0 + scale_6

        # pd_op.full: (1xi32) <- ()
        full_15 = paddle._C_ops.full([1], float('-1'), paddle.int32, paddle.core.CPUPlace())

        # builtin.combine: ([-1x12096x2xf32, -1x12096x2xf32]) <- (-1x12096x2xf32, -1x12096x2xf32)
        combine_11 = [subtract_0, add_1]

        # pd_op.concat: (-1x12096x4xf32) <- ([-1x12096x2xf32, -1x12096x2xf32], 1xi32)
