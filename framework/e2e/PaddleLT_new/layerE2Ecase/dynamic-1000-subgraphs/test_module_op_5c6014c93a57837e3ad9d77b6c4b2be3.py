...
class BlockEntries:
    ...
    def builtin_module_0_0_0(self, data_1, data_2, data_0):
        ...
        unsqueeze_0, unsqueeze_1 = paddle.unsqueeze(data_1, full_int_array_0), None
        ...
        split_4 = paddle.split(data_2, full_int_array_1, assign_0)
        ...
        unsqueeze_2, unsqueeze_3 = paddle.unsqueeze(split_5, full_int_array_2), None
        ...
        pow_0 = paddle._C_ops.pow(sum_0, 2)

        # pd_op.pow: (-1x-1x-1xf32) <- (-1x-1x-1xf32)
        pow_1 = paddle._C_ops.pow(sqrt_0, 3)
        ...
        scale_0 = paddle._C_ops.scale(multiply_4, full_1, 1e-09, True)

        # pd_op.divide: (-1x-1x-1xf32) <- (-1x-1x-1xf32, -1x-1x-1xf32)
        divide_0 = pow_0 / scale_0

        # pd_op.pow: (-1x-1x-1xf32) <- (-1x-1x-1xf32)
        pow_2 = paddle._C_ops.pow(sum_1, 2)

        # pd_op.pow: (-1x-1x-1xf32) <- (-1x-1x-1xf32)
        pow_3 = paddle._C_ops.pow(sqrt_1, 3)

        # pd_op.multiply: (-1x-1x-1xf32) <- (-1x-1x-1xf32, -1x-1x1xf32)
        multiply_5 = pow_3 * min_0

        # pd_op.scale: (-1x-1x-1xf32) <- (-1x-1x-1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(multiply_5, assign_6, 1e-09, True)
        ...
        # pd_op.full: (1xf32) <- ()
        full_2 = paddle._C_ops.full([1], -6, paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1x-1x-1xf32) <- (-1x-1x-1xf32, 1xf32)
        scale_2 = paddle._C_ops.scale(add_0, full_2, 0, True)

        # pd_op.exp: (-1x-1x-1xf32) <- (-1x-1x-1xf32)
        exp_0 = paddle._C_ops.exp(scale_2)

        # pd_op.full: (1xf32) <- ()
        full_3 = paddle._C_ops.full([1], 0.0833333, paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1x-1x1xf32) <- (-1x-1x1xf32, 1xf32)
        scale_3 = paddle._C_ops.scale(min_0, full_3, 0, True)

        # pd_op.full: (1xf32) <- ()
        full_4 = paddle._C_ops.full([1], 6.28319, paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1x-1x1xf32) <- (-1x-1x1xf32, 1xf32)
        scale_4 = paddle._C_ops.scale(scale_3, full_4, 0, True)

        # pd_op.scale: (-1x-1x1xf32) <- (-1x-1x1xf32, 1xf32)
        scale_5 = paddle._C_ops.scale(scale_4, assign_5, 1e-09, True)
