...
class BlockEntries:
    ...
    def builtin_module_59_0_0(self, data_0, data_1):
        ...
        # pd_op.full: (xf32) <- ()
        full_0 = paddle._C_ops.full([], float('0.111111'), paddle.float32, paddle.framework._current_expected_place())
        ...
        scale_0 = paddle._C_ops.scale(abs_0, full_1, float('0'), True)

        # pd_op.multiply: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        multiply_0 = scale_0 * abs_0

        # pd_op.full: (1xf32) <- ()
        full_2 = paddle._C_ops.full([1], float('9'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1x-1xf32) <- (-1x-1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(multiply_0, full_2, float('0'), True)

        # pd_op.full: (1xf32) <- ()
        full_3 = paddle._C_ops.full([1], float('1'), paddle.float32, paddle.core.CPUPlace())

        # pd_op.scale: (-1x-1xf32) <- (-1x-1xf32, 1xf32)
        scale_2 = paddle._C_ops.scale(abs_0, full_3, float('-0.0555556'), True)
