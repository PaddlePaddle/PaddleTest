...
class RandomNet(nn.Layer):
    def __init__(self):
        ...
        self.layers = nn.LayerList(
            [
                nn.Conv2D(3, 73, kernel_size=[3, 3], padding=1),
                nn.BatchNorm2D(num_features=73, momentum=0.9, epsilon=1e-05),
                nn.ReLU(),
                nn.Conv2D(73, 125, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
        ...
        self.fc_layers = nn.LayerList(
            [
                nn.Linear(in_features=128000, out_features=604),
                nn.ReLU(),
                nn.Linear(in_features=604, out_features=710),
                nn.ReLU(),
                nn.Linear(in_features=710, out_features=10),
            ]
        )
...
if __name__ == "__main__":
    ...
    path = f"simple/model8/demo8"  # 路径这里用demo，若改infer中对应需要改
