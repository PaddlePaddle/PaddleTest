...
class RandomNet(nn.Layer):
    def __init__(self):
        ...
        self.layers = nn.LayerList(
            [
                nn.Conv2D(3, 96, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(96, 123, kernel_size=[3, 3], padding=1),
                nn.BatchNorm2D(num_features=123, momentum=0.9, epsilon=1e-05),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(123, 84, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.Conv2D(84, 101, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
            ]
        )

        self.fc_layers = nn.LayerList([nn.Linear(in_features=25856, out_features=10)])
...
if __name__ == "__main__":
    ...
    path = f"simple/model3/demo3"  # 路径这里用demo，若改infer中对应需要改
