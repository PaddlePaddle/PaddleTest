...
class RandomNet(nn.Layer):
    def __init__(self):
        ...
        self.layers = nn.LayerList(
            [
                nn.Conv2D(3, 109, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.Conv2D(109, 120, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(120, 103, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
            ]
        )
        ...
        self.fc_layers = nn.LayerList(
            [
                nn.Linear(in_features=105472, out_features=588),
                nn.ReLU(),
                nn.Linear(in_features=588, out_features=143),
                nn.ReLU(),
                nn.Linear(in_features=143, out_features=10),
            ]
        )
...
if __name__ == "__main__":
    ...
    path = f"simple/model4/demo4"  # 路径这里用demo，若改infer中对应需要改
