...
def test_non_stream_with_logprobs():
    ...
    if os.getenv("BASELINE") == "1":
        with open(f"/MODELDATA/base_logprobs_mtp_{TEST_BRANCH}_non_stream.txt", "w", encoding="utf-8") as f:
    ...
    if os.getenv("AGILE_COMPILE_BRANCH") == "release/online/20251131":
        ...
        with open(f"/MODELDATA/base_logprobs_mtp_{TEST_BRANCH}_non_stream.txt", "r", encoding="utf-8") as f:
            baseline = json.load(f)
        assert logprobs == baseline, f"logprobs不一致,result:{logprobs}, baseline:{baseline}"
...
def test_stream_with_logprobs():
    ...
    print(logprobs)
    if os.getenv("BASELINE") == "1":
        with open(f"/MODELDATA/base_logprobs_mtp_{TEST_BRANCH}_stream.txt", "w", encoding="utf-8") as f:
            json.dump(logprobs, f, ensure_ascii=False, indent=2, sort_keys=True)
    ...
    if os.getenv("AGILE_COMPILE_BRANCH") == "release/online/20251131":
        ...
        with open(f"/MODELDATA/base_logprobs_mtp_{TEST_BRANCH}_stream.txt", "r", encoding="utf-8") as f:
            baseline = json.load(f)
        assert logprobs == baseline, f"logprobs不一致,result:{logprobs}, baseline:{baseline}"
