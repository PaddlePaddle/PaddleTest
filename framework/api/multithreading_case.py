...
def runCETest(params):
    ...
    case = params[1]
    print("case: %s" % case)
    val = os.system("export FLAGS_call_stack_level= && cd %s && python3 -m pytest %s" % (path, case))
    retry_count = 0
    final_result = ""
    while val != 0:
        val = os.system("export FLAGS_call_stack_level=2 && cd %s && python3 -m pytest %s" % (path, case))
