...
class RandomNet(nn.Layer):
    ...
    def __init__(self):
        ...
        self.layers = nn.LayerList(
            [
                nn.Conv2D(3, 124, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.Conv2D(124, 83, kernel_size=[3, 3], padding=1),
                nn.BatchNorm2D(num_features=83, momentum=0.9, epsilon=1e-05),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(83, 84, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(84, 88, kernel_size=[3, 3], padding=1),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
                nn.Conv2D(88, 71, kernel_size=[3, 3], padding=1),
                nn.BatchNorm2D(num_features=71, momentum=0.9, epsilon=1e-05),
                nn.ReLU(),
                nn.MaxPool2D(kernel_size=2, stride=2, padding=0),
            ]
        )
        ...
        self.fc_layers = nn.LayerList(
            [nn.Linear(in_features=1136, out_features=345), nn.ReLU(), nn.Linear(in_features=345, out_features=10)]
        )
