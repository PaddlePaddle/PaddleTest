...
obj = TestMaxPool3d(paddle.nn.layer.MaxPool3D)
...
@pytest.mark.api_nn_MaxPool3D_vartype
...
@pytest.mark.api_nn_MaxPool3D_parameters
def test_max_pool3d():
    """
    default
    """
...
def test_max_pool3d1():
    ...
    ceil_mode = True
    """
    x = randtool("float", -10, 10, [2, 3, 4, 4, 4])
    kernel_size = [3, 3, 3]
    stride = [1, 1, 1]
    padding = [0, 0, 0]
    ceil_mode = True
...
def test_max_pool3d2():
    ...
    obj.run(res=res, data=x, kernel_size=kernel_size, stride=stride, padding=padding, data_format=data_format)
