...
def test_cosine_similarity2():
    """
    modify value range, the elements of input tensor have
...
@pytest.mark.api_nn_CosineSimilarity_parameters
def test_cosine_similarity7():
    """
    the two input tensors don't have the same shape,
    but are broadcastable and don't have the same data type
    """
    x1 = randtool("float", -10, 10, [5, 6, 7, 8, 10])
    x2 = randtool("int", -10, 10, [7, 8, 10])
    axis = -1
    eps = 1e-8
    expected_result = compute_cosine_similarity(x1, x2, axis=axis, eps=eps)
