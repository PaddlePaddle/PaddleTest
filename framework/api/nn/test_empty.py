...
dtypes = [None, np.bool, np.float16, np.float32, np.float64, np.int32, np.int64, bool, 'float16', 'float32',
          'float64', 'int32', 'int64']
            ...
            with fluid.program_guard(main_program=main_program, startup_program=startup_program):
                for place in places:
                    for dtype in dtypes:
                        ...
                        assert np.allclose(np.array(static_out).shape, [1, 3, 3, 2], atol=0.005, rtol=0.05,
                                           equal_nan=True)
