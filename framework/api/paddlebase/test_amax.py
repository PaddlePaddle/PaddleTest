...
def cal_amax(x, jud=True, axis=None, keepdim=False):
    """
    calculate amin
    """
    res = np.amax(x, axis=axis, keepdims=keepdim)
...
def cal_api(x, dtype="float32", axis=None, keepdim=False):
    ...
    dynamic_res = paddle.amax(xp, axis=axis, keepdim=keepdim)
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(main_program=main_program, startup_program=startup_program):
            ...
            feed = {"s0": x}
            out = paddle.amax(data0, axis=axis, keepdim=keepdim)
            if dtype == "float32" or dtype == "float64":
...
@pytest.mark.api_base_amax_parameters
def test_amax1():
    """
    x: 3d-tensor
    """
...
def test_amax3():
    ...
    np_res, np_grad = cal_amax(x, axis=-1, keepdim=True)
...
def test_amax4():
    ...
    """
    x = randtool("float", -40, 140, (3, 2, 5, 4))
    np_res, np_grad = cal_amax(x, axis=2, keepdim=True)
...
def test_amax5():
    ...
    """
    x = randtool("float", -40, 140, (3, 2, 5, 4))
    np_res, np_grad = cal_amax(x, axis=(1, 2), keepdim=True)
