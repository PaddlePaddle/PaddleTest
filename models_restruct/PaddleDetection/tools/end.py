...
class PaddleDetection_End(object):
    ...
    def __init__(self):
        ...
        self.log_path = os.path.join(
            os.getcwd(), self.log_dir, self.reponame, self.qa_model_name, self.log_name)
    ...
    def build_end(self):
        ...
        """
        # kill遗留程序
        logger.info("PID before is {}".format(
            os.system(f"ps aux| grep '{self.qa_model_name}'| grep -v 'main.py'")))
        cmd_kill = os.system(
        ...
        logger.info("PID after is {}".format(
            os.system(f"ps aux| grep '{self.qa_model_name}'| grep -v 'main.py'")))
        # 异常256
