...
class PaddleOCR_Build(Model_Build):
    ...
    def prepare_opencv(self):
        ...
        # cmake
        print(os.getcwd())
        cmd='cmake .. -DCMAKE_INSTALL_PREFIX=%s \
    ...
    -DWITH_JPEG=ON -DBUILD_JPEG=ON -DWITH_PNG=ON -DBUILD_PNG=ON -DWITH_TIFF=ON -DBUILD_TIFF=ON' % (install_path)
    ...
    def prepare_c_predict_library(self):
        print(os.getcwd())
        os.chdir('PaddleOCR/deploy/cpp_infer')
        wget.download('https://paddle-inference-lib.bj.bcebos.com/2.3.2/cxx_c/Linux/GPU/x86-64_gcc8.2_avx_mkl_cuda10.2_cudnn8.1.1_trt7.2.3.4/paddle_inference.tgz')
    ...
    def compile_c_predict_demo(self):
        ...
        os.chdir('build')
        print(os.getcwd())
        cmd='cmake .. -DPADDLE_LIB=%s -DWITH_MKL=ON -DWITH_GPU=OFF -DWITH_STATIC_LIB=OFF -DWITH_TENSORRT=OFF \
    -DOPENCV_DIR=%s -DCUDNN_LIB=%s -DCUDA_LIB=%s -DTENSORRT_DIR=%s' % (LIB_DIR, OPENCV_DIR, CUDNN_LIB_DIR, CUDA_LIB_DIR, TENSORRT_DIR)
        print(cmd)
        repo_result = subprocess.getstatusoutput(cmd)
        exit_code = repo_result[0]
        output = repo_result[1]
