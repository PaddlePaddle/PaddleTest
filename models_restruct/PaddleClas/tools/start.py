...
class PaddleClas_Start(object):
    ...
    def download_infer_tar(self, value=None):
        ...
        if os.path.exists(value) and os.path.exists(value.replace(".tar")):
            ...
            self.download_data("https://paddle-imagenet-models-name.bj.bcebos.com/\
                dygraph/rec/models/inference/{}.tar".format(value))
        os.chdir(path_now)
        return 0
    ...
    def prepare_predict_env(self):
        ...
        if self.model_type == "ImageNet" or self.model_type == "slim" or self.model_type == "PULC" or \
            self.model_type == "DeepHash":
            #定义变量
    ...
    def prepare_creat_yaml(self):
        ...
        if self.mode == "function":
            if os.path.exists(os.path.join\
                ("cases", self.qa_yaml_name)) == True: #cases 是基于最原始的路径的
                os.remove(os.path.join("cases", self.qa_yaml_name)) #删除已有的 precision 用 base
        else:
            if os.path.exists(os.path.join\
                ("cases", self.qa_yaml_name)) == False: #cases 是基于最原始的路径的
                source_yaml_name = self.base_yaml_dict[self.model_type]
                try:
                    shutil.copy(os.path.join("cases", source_yaml_name), \
