...
def parse_args():
    ...
    parser.add_argument("--config", type=str, help="The path of config file.", default="dy2st.yaml")
...
def analysis(log_dir, config, train_or_eval):
    ...
    for mode in modes:
        ...
        print('###log_info_map[mode]', log_info_map[mode])
    print('###log_info_map', log_info_map)
...
if __name__ == "__main__":
    ...
    f.close()
    config = config["ppcls^configs^CAE^cae_base_patch16_224_finetune_dy2st_convergence"]
    # print('###config', config)
    # input()
    train_or_eval = "train"
