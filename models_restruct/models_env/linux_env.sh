...
if [[ ${AGILE_PIPELINE_NAME} =~ "Cuda102" ]];then
    ...
    if [[ ${AGILE_PIPELINE_NAME} =~ "Centos" ]];then
        ...
        export Image_version=${Image_version:-"registry.baidubce.com/paddlepaddle/paddle:latest-dev-cuda11.2-cudnn8.2-trt8.0-gcc82"}
    fi
    ...
    if [[ ${AGILE_PIPELINE_NAME} =~ "Centos" ]];then
        ...
        export Image_version=${Image_version:-"registry.baidubce.com/paddlepaddle/paddle:latest-dev-cuda11.6-cudnn8.4-trt8.4-gcc82"}
    fi
