...
if [[ "${docker_flag}" == "" ]]; then
    ...
    NV_GPU=${set_cuda_back} nvidia-docker run -i   --rm \
        ...
        export qa_yaml_name=configs^petr^petrv2_vovnet_gridmask_p4_800x320_dn_amp.yml
        export rd_yaml_path=configs/petr/petrv2_vovnet_gridmask_p4_800x320_dn_amp.yml
        export reponame=Paddle3D
        export system=linux
        export step=train:dygraph2static_amp_prim_cinn_test
        export paddle_whl=null
        export mode=function
        python tools/end.py
        python -m pytest -sv test_plot.py
    ' &
    wait $!
    exit $?
fi
