...
mv PaddleTest task
cp -r ./task/models_restruct/PaddleLLM/. ./TestFrameWork/
rm -rf task
####    套件库下载    #####
...
sed -i "s/1200/10/g" train/grpo.sh
sed -i "s/1200/10/g" train/reinforce_plus_plus.sh
...
# grpo
python main.py --models_file='tools/PaddleLLM_grpo' --step="${step:-train}" --reponame="${reponame:-PaddleClas}" --use_build="${use_build:-yes}" --paddle_whl="${paddle_whl:-None}" --set_cuda='0,1,2,3' --timeout="${timeout:-3600}"  --plot='True' > run_grpo.log 2>&1 &
grpo_pid=$!
# rf++
python main.py --models_file='tools/PaddleLLM_rf++' --step="${step:-train}" --reponame="${reponame:-PaddleClas}" --use_build="${use_build:-yes}" --paddle_whl="${paddle_whl:-None}" --set_cuda='4,5,6,7' --timeout="${timeout:-3600}"  --plot='True' > run_rf++.log 2>&1 &
rf_pid=$!
