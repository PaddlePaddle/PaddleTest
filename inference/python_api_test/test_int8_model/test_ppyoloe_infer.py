...
def draw_box(image_file, results, class_label, threshold=0.5):
    ...
    for i in range(len(results)):
        ...
        if conf < threshold:
            continue
        xmin, ymin, xmax, ymax = int(results[i, 2]), int(results[i, 3]), int(results[i, 4]), int(results[i, 5])
...
def predict_image(predictor, image_file, image_shape=[640, 640], warmup=1, repeats=1, threshold=0.5):
    ...
    res_img = draw_box(image_file, res["bbox"], CLASS_LABEL, threshold=threshold)
...
def main():
    ...
    if FLAGS.image_file:
        ...
        predict_image(
            predictor, FLAGS.image_file, image_shape=[FLAGS.img_shape, FLAGS.img_shape], warmup=warmup, repeats=repeats
        )
    else:
