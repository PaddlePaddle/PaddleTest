...
class YOLOPostProcess(object):
    ...
    def __init__(self, score_threshold=0.25, nms_threshold=0.5, multi_label=False, keep_top_k=300):
    ...
    def _non_max_suppression(self, prediction):
        ...
        for batch_id, boxes in enumerate(prediction):
            ...
            if self.multi_label:
                ...
                boxes = np.concatenate((convert_box[i], boxes[i, j + 5, None], j[:, None].astype(np.float32)), axis=1)
            else:
    ...
    def __call__(self, outs, scale_factor):
        ...
        for i, pred in enumerate(preds):
            ...
            pred_bboxes /= scale
            bbox = np.concatenate([pred[:, -1][:, np.newaxis], pred[:, -2][:, np.newaxis], pred_bboxes], axis=-1)
            bboxs.append(bbox)
...
def coco_metric(anno_file, bboxes_list, bbox_nums_list, image_id_list):
    ...
    try:
        ...
        print("[ERROR] Not found pycocotools, please install by `pip install pycocotools`")
        sys.exit(1)
    ...
    results = []
    for bboxes, bbox_nums, image_id in zip(bboxes_list, bbox_nums_list, image_id_list):
...
def _get_det_res(bboxes, bbox_nums, image_id, label_to_cat_id_map):
    ...
    for i, _ in enumerate(bbox_nums):
        ...
        for j in range(det_nums):
            ...
            h = ymax - ymin
            bbox = [xmin, ymin, w, h]
            dt_res = {"image_id": cur_image_id, "category_id": category_id, "bbox": bbox, "score": score}
