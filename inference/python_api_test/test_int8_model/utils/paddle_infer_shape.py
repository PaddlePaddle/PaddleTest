...
def _process_old_ops_desc(program):
...
def infer_shape(program, input_shape_dict):
    ...
    if model_version != paddle_version:
        print(
            "[WARNING] The model is saved by paddlepaddle v{}, but now your paddlepaddle "
            "is version of {}, this difference may cause error, it is recommend you reinstall "
            "a same version of paddlepaddle for this model".format(model_version, paddle_version)
        )
...
def parse_arguments():
    ...
    parser.add_argument("--save_dir", required=True, help="Path of directory to save the new exported model.")
...
if __name__ == "__main__":
    ...
    [prog, ipts, outs] = fluid.io.load_inference_model(
        args.model_dir, exe, model_filename=args.model_filename, params_filename=args.params_filename
    )
    _process_old_ops_desc(prog)
    infer_shape(prog, input_shape_dict)
    fluid.io.save_inference_model(
        args.save_dir, ipts, outs, exe, prog, model_filename=args.model_filename, params_filename=args.params_filename
    )
