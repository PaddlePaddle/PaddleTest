...
class PPYOLOEPostProcess(object):
    ...
    def __init__(self, score_threshold=0.4, nms_threshold=0.5, nms_top_k=10000, keep_top_k=300):
    ...
    def _non_max_suppression(self, prediction, scale_factor):
        ...
        for batch_id in range(batch_size):
            bboxes, confidences = prediction[batch_id][..., :4], prediction[batch_id][..., 4:]
            # nms
            picked_box_probs = []
            picked_labels = []
            for class_index in range(0, confidences.shape[1]):
                ...
                box_probs = hard_nms(box_probs, iou_threshold=self.nms_threshold, top_k=self.nms_top_k)
            ...
            if len(picked_box_probs) == 0:
                ...
                if out_box.shape[0] > self.keep_top_k:
                    out_box = out_box[out_box[:, 1].argsort()[::-1][: self.keep_top_k]]
    ...
    def __call__(self, outs, scale_factor):
        out_boxes_list, box_num_list = self._non_max_suppression(outs, scale_factor)
