...
def argsparser():
    ...
    parser.add_argument("--arch", type=str, default="YOLOv5", help="architectures name.")
...
def draw_box(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    ...
    for i, _ in enumerate(boxes):
        ...
        cv2.putText(img, text, (x0, y0 + txt_size[1]), font, 0.8, (0, 255, 0), thickness=2)
...
def load_predictor(
    ...
    if precision in precision_map.keys() and use_trt:
        ...
        if use_dynamic_shape:
            dynamic_shape_file = os.path.join(FLAGS.model_path, "dynamic_shape.txt")
...
def infer(predictor):
    ...
    input_image, scale_factor = preprocess(origin_img, [FLAGS.img_shape, FLAGS.img_shape])
    ...
    dets = res["bbox"]
    if dets is not None:
        final_boxes, final_scores, final_class = dets[:, 2:], dets[:, 1], dets[:, 0]
        res_img = draw_box(origin_img, final_boxes, final_scores, final_class, conf=0.5, class_names=CLASS_LABEL)
