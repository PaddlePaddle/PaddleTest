...
PADDLE_WHL=${PADDLE_WHL:-https://paddle-qa.bj.bcebos.com/paddle-pipeline/Develop-GpuAll-LinuxCentos-Gcc82-Cuda112-Trtoff-Py38-Compile/latest/paddlepaddle_gpu-0.0.0-cp38-cp38-linux_x86_64.whl}
FRAME=${FRAME:-paddle}
FRAME_BRANCH=${FRAME_BRANCH:-develop}
FRAME_VERSION=${FRAME_VERSION:-0.0.0}
DEVICE=${DEVICE:-T4}
MODE=${MODE:-mkldnn_int8,mkldnn_fp32}
...
python get_benchmark_info.py ${FRAME} ${FRAME_BRANCH} ${PADDLE_COMMIT} ${FRAME_VERSION} ${DOCKER_IMAGE} ${PYTHON_VERSION} ${CUDA_VERSION} ${CUDNN_VERSION} ${DEVICE} ${MODE} ${METRIC} ${SAVE_FILE}
