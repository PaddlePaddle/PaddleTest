...
def check_model_exist():
    ...
    check model exist
    """
    unet_url = "https://paddle-qa.bj.bcebos.com/inference_model/2.6/seg/unet.tgz"
    if not os.path.exists("./unet/model.pdiparams"):
        wget.download(unet_url, out="./")
        tar = tarfile.open("unet.tgz")
        tar.extractall()
        tar.close()
...
def test_trt_fp32_more_bz():
    """
    compared trt fp32 batch_size=1-2 unet outputs with true val
    """
    check_model_exist()
...
def test_jetson_trt_fp32_more_bz():
    """
    compared trt fp32 batch_size=1 unet outputs with true val
    """
    check_model_exist()
...
def test_trt_fp32_bz1_multi_thread():
    """
    compared trt fp32 batch_size=1 unet multi_thread outputs with true val
    """
    check_model_exist()
