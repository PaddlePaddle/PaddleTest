...
def test_more_bz_mkldnn():
    """
    compared mkldnn solov2 batch size = [1,4,8,10] outputs with true val
    """
    check_model_exist()
    ...
    for batch_size in batch_size_pool:
        ...
        result = npy_list[0 : batch_size * 4]
        ...
        scale_0 = []
        for batch in range(0, batch_size * 4, 4):
            scale_0 = np.concatenate((scale_0, result[batch].flatten()), axis=0)
        scale_1 = []
        for batch in range(1, batch_size * 4, 4):
            scale_1 = np.concatenate((scale_1, result[batch].flatten()), axis=0)
        scale_2 = []
        for batch in range(1, batch_size * 4, 4):
            scale_1 = np.concatenate((scale_2, result[batch].flatten()), axis=0)
        scale_3 = []
        for batch in range(1, batch_size * 4, 4):
            scale_1 = np.concatenate((scale_3, result[batch].flatten()), axis=0)

        output_data_dict = {"save_infer_model/scale_0.tmp_1": scale_0, "save_infer_model/scale_1.tmp_1": scale_1,
        "save_infer_model/scale_0.tmp_2": scale_2,"save_infer_model/scale_0.tmp_4": scale_3}
        test_suite.load_config(model_file="./solov2/model.pdmodel", params_file="./solov2/model.pdiparams")
...
def test_gpu_more_bz():
    """
    compared gpu solov2 batch size = [1,4,8] outputs with true val
    """
    check_model_exist()
