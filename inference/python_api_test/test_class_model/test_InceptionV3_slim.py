...
def check_model_exist():
    ...
    check model exist
    """
    resnet50_slim_url = "https://paddle-qa.bj.bcebos.com/PaddleSlim/ACT_models/InceptionV3_act_qat.tar"
    if not os.path.exists("./InceptionV3_act_qat/inference.pdmodel"):
        wget.download(resnet50_slim_url, out="./")
        tar = tarfile.open("InceptionV3_act_qat.tar")
        tar.extractall()
        tar.close()
...
def test_config():
    ...
    test_suite.load_config(
        model_file="./InceptionV3_act_qat/inference.pdmodel", params_file="./InceptionV3_act_qat/inference.pdiparams"
    )
...
def test_disable_gpu():
    ...
    batch_size = 1
    fake_input = np.random.randn(batch_size, 3, 299, 299).astype("float32")
...
def test_trt_int8_more_bz():
    ...
    for batch_size in batch_size_pool:
        ...
        if "win" in sys.platform:
            test_suite.load_config(
                model_file=".\\InceptionV3_act_qat\\inference.pdmodel",
                params_file=".\\InceptionV3_act_qat\\inference.pdiparams",
            )
        else:
            test_suite.load_config(
                model_file="./InceptionV3_act_qat/inference.pdmodel",
        ...
        test_suite.collect_shape_info(
            model_path="./InceptionV3_act_qat/", input_data_dict=input_data_dict, device="gpu"
        )
        del test_suite
