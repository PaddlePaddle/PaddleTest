...
def test_disable_gpu():
    ...
    test_suite.load_config(model_file="./StrucText/model", params_file="./StrucText/params")
    input_data_dict = {}
    input_data_dict["images"] = np.ones((1, 3, 512, 512)).astype(np.float32)
    input_data_dict["seq_token"] = np.ones((1, 512)).astype(np.int64)
    input_data_dict["seq_ids"] = np.ones((1, 512)).astype(np.int64)
    input_data_dict["seq_mask"] = np.ones((1, 512)).astype(np.int64)
    input_data_dict["seq_bboxes"] = np.ones((1, 512, 4)).astype(np.float32)
...
def test_gpu_more_bz():
    """
    compared gpu batch_size=1 StrucText outputs with true val
    """
    check_model_exist()
