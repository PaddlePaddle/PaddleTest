...
class YamlLoader(object):
    ...
    def get_solution_dir(self, n):
        ...
        return pp.get("solution_metric_dir"), pp.get("solution_loss_dir")
    
    def get_run_time(self, n):
        pp = self.yml[n].get("Post-processing")
        return  pp.get("run_time")
...
class GenerateOrder(object):
    ...
    def get_order(self, py, model_name, case_num):
        """get order"""
        order = "python3.7 {} >test_{}.log".format(py,model_name)
        return order

    def __call__(self, py, model_name, case_num):
        """call"""
        return self.get_order(py, model_name, case_num)
...
def compare(res, expect_time, find_str="'train' took"):
    """
    比较函数
    """
    res_time = float('inf')
    with open(res, "r", encoding="utf-8") as f:
        for line in f:
            if find_str in line:
                cut_res = line.split()
                res_time = float(cut_res[-2])
                break
    print("res_time="+str(res_time))
    print("expect_time="+str(expect_time))
    print((res_time - expect_time)/expect_time)
    assert((res_time - expect_time)/expect_time <= 0.5)
