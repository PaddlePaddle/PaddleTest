...
def compute_bc_loss(inputs_attr, labels_attr, outputs_var, pde_disc):
    ...
    for i, name_b in enumerate(inputs_attr["bc"].keys()):
        ...
        for j in range(len(pde_disc.bc[name_b])):
            ...
            bc_loss += l2_norm_square((out_el[:, index] - rhs_b) * np.sqrt(wgt_b), 10000)
    return bc_loss
...
def compute_eq_loss(inputs, outputs, labels_var):
    ...
    hes0, = paddle.static.gradients([jac0[:, 0]], [inputs])  # du*du/dx*dx, du*du/dx*dy, du*du/dx*dz
    hes1, = paddle.static.gradients([jac0[:, 1]], [inputs])  # du*du/dy*dx, du*du/dy*dy, du*du/dy*dz
    hes2, = paddle.static.gradients([jac0[:, 2]], [inputs])  # du*du/dz*dx, du*du/dz*dy, du*du/dz*dz
    hes3, = paddle.static.gradients([jac1[:, 0]], [inputs])  # dv*dv/dx*dx, dv*dv/dx*dy, dv*dv/dx*dz
    hes4, = paddle.static.gradients([jac1[:, 1]], [inputs])  # dv*dv/dy*dx, dv*dv/dy*dy, dv*dv/dy*dz
    hes5, = paddle.static.gradients([jac1[:, 2]], [inputs])  # dv*dv/dz*dx, dv*dv/dz*dy, dv*dv/dz*dz
    hes6, = paddle.static.gradients([jac2[:, 0]], [inputs])  # dw*dw/dx*dx, dw*dw/dx*dy, dw*dw/dx*dz
    hes7, = paddle.static.gradients([jac2[:, 1]], [inputs])  # dw*dw/dy*dx, dw*dw/dy*dy, dw*dw/dy*dz
    hes8, = paddle.static.gradients([jac2[:, 2]], [inputs])  # dw*dw/dz*dx, dw*dw/dz*dy, dw*dw/dz*dz
...
def compile_and_convert_back_to_program(
    program=None, feed=None, fetch_list=None, fetch_var_name="fetch", scope=None, use_prune=False, loss_name=None
):
    """
    func6
    """
    ...
    def _add_fetch_ops(program, fetch_list, fetch_var_name):
        ...
        if fetch_var_name in global_block.vars:
            ...
            fetch_var = global_block.create_var(
                name=fetch_var_name, type=core.VarDesc.VarType.FETCH_LIST, persistable=True
            )

        # append fetch_operators
        if not fluid.executor.has_fetch_operators(global_block, fetch_list, fetch_var_name, "fetch"):
            for i, var in enumerate(fetch_list):
                assert isinstance(var, Variable) or isinstance(
                    var, six.string_types
                ), "Wrong type for fetch_list[%s]: %s" % (i, type(var))
                global_block.append_op(
                    type="fetch", inputs={"X": [var]}, outputs={"Out": [fetch_var]}, attrs={"col": i}
                )
        return tmp_program
...
def set_init_dist_attr(serial_main_prog):
    ...
    set_var_dist_attr(default_dist_context, x_tensor, [-1, -1], _global_process_mesh, mark_annotated=True)
    set_var_dist_attr(default_dist_context, bc_idx_tensor, [-1], _global_process_mesh, mark_annotated=True)
