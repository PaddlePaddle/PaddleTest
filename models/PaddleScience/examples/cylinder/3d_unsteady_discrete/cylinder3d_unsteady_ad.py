"""
cylinder3d_unsteady_ad ce test
"""
...
from utils import (
    ...
    compute_bc_loss,
    compute_eq_loss,
    compile_and_convert_back_to_program,
    create_inputs_var,
    create_labels_var,
    convert_to_distributed_program,
...
# pde discretization
pde_disc = pde.discretize(time_method="implicit", time_step=1, geo_disc=geo_disc)
...
with paddle.static.program_guard(main_program, startup_program):
    ...
    # eq loss
    output_var_0_eq_loss = compute_eq_loss(inputs_var[0], outputs_var[0], labels_var[0:3])

    output_var_4_eq_loss = compute_eq_loss(inputs_var[4], outputs_var[4], labels_var[7:10])
    # data_loss
    data_loss = (
        l2_norm_square(outputs_var[4][:, 0] - labels_var[3])
    ...
    # total_loss
    total_loss = paddle.sqrt(bc_loss + output_var_0_eq_loss + output_var_4_eq_loss + 100.0 * data_loss)
...
main_program = compile_and_convert_back_to_program(
    main_program, feed=feeds, fetch_list=fetches, use_prune=True, loss_name=total_loss.name
)
...
for i in range(num_time_step):
    ...
    self_lables = algo.feed_data_user_next(self_lables, labels_attr, GetRealPhyInfo(next_time, need_info="physic"))
    self_lables = data_parallel_partition(self_lables, time_step=i)
    len1 = len(self_lables)
    for j in range(len1):
