...
def test_loss_L2_0():
    ...
    net = psci.network.FCNet(num_ins=2, num_outs=1, num_layers=2, hidden_size=20, dtype="float32", activation="tanh")
    for i in range(2):
...
def test_loss_L2_2():
    ...
    obj.run(pdes=pdes, geo=geo, net=net, aux_func=RighthandBatch, eq_weight=0.5)
...
def test_loss_L2_3():
    ...
    obj.run(pdes=pdes, geo=geo, net=net, aux_func=RighthandBatch, eq_weight=0.5, bc_weight=bc_weight)
...
def test_loss_L2_5():
    ...
    """
    geo = psci.geometry.Rectangular(space_origin=(-0.05, -0.05), space_extent=(0.05, 0.05))
    ...
    net = psci.network.FCNet(num_ins=2, num_outs=3, num_layers=2, hidden_size=4, dtype="float32", activation="tanh")
    for i in range(2):
        net.weights[i] = paddle.ones_like(net.weights[i])

    obj.run(pdes=pdes, geo=geo, net=net, eq_weight=0.01, bc_weight=None, synthesis_method="norm")
