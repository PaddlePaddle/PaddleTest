...
def test_ldc2d_unsteady_re10_exit_code():
    ...
    command = f"python{py_version} ../../examples/ldc/ldc2d_unsteady_Re10.py \
...
@allure.title("导出功能测试")
def test_ldc2d_unsteady_re10_export():
    """
    测试函数：测试 ldc2d_unsteady_re10.py 脚本的export功能
    """
    py_version = os.getenv("py_version", "3.8")  # Python 版本号，从环境变量中获取，默认值为3.8
...
@allure.title("推理功能测试")
def test_ldc2d_unsteady_re10_infer():
    """
    测试函数：测试 ldc2d_unsteady_re10.py 脚本的infer功能
    """
    py_version = os.getenv("py_version", "3.8")  # Python 版本号，从环境变量中获取，默认值为3.8
