...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    ...
    python -u infer.py -m config_bigdata.yaml > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_dy_gpu2" ];then #多卡
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    # 多卡的运行方式
    python -m paddle.distributed.launch infer.py -m config_bigdata.yaml > ${log_path}/$2.log 2>&1
    print_info $? $2
    ...
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    python -u static_infer.py -m config_bigdata.yaml > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_st_gpu2" ];then #多卡
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    # 多卡的运行方式
    python -m paddle.distributed.launch static_infer.py -m config_bigdata.yaml > ${log_path}/$2.log 2>&1
    print_info $? $2
