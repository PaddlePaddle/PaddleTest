...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    ...
    python -u ../../../tools/infer.py -m config_bigdata.yaml -o runner.infer_load_path="output_model_simnet_all_dy_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt
    ...
    sed -i "s/  use_gpu: True/  use_gpu: False/g" config_bigdata.yaml
    python -u ../../../tools/static_infer.py -m config_bigdata.yaml -o runner.infer_load_path="output_model_simnet_all_st_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt
else
