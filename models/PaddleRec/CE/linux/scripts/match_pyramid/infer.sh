...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    ...
    python -u ../../../tools/infer.py -m config.yaml -o runner.infer_load_path="output_model_pyramid_all_dy_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt;
    ...
    python -u ../../../tools/static_infer.py -m config.yaml -o runner.infer_load_path="output_model_pyramid_all_st_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt;

elif [ "$1" = "mac_dy_cpu" ];then
    python -u ../../../tools/infer.py -m config.yaml -o runner.infer_load_path="output_model_pyramid_all_mac_dy_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt;

elif [ "$1" = "mac_st_cpu" ];then
    python -u ../../../tools/static_infer.py -m config.yaml -o runner.infer_load_path="output_model_pyramid_all_mac_st_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    rm -rf result.txt;
    cp ${log_path}/S_$2.log result.txt;
...
python eval.py > ${log_path}/$2.log 2>&1
print_info $? $2
