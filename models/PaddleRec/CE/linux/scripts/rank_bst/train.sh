...
log_path=$root_path/log/rank_bst/
mkdir -p $log_path
#临时环境更改
...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    ...
    python -u ../../../tools/trainer.py -m config.yaml -o runner.model_save_path="output_model_bst_all_dy_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
    ...
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    # 多卡的运行方式
    python -m paddle.distributed.launch ../../../tools/static_trainer.py -m config_bigdata.yaml -o runner.model_save_path="output_model_bst_all_st_gpu2" > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_st_cpu" ];then
    python -u ../../../tools/static_trainer.py -m config.yaml -o runner.model_save_path="output_model_bst_all_st_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
# mac small_data train
elif [ "$1" = "mac_dy_cpu" ];then
    python -u ../../../tools/trainer.py -m config.yaml -o runner.model_save_path="output_model_bst_all_mac_dy_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
elif [ "$1" = "mac_st_cpu" ];then
    python -u ../../../tools/static_trainer.py -m config.yaml -o runner.model_save_path="output_model_bst_all_mac_st_cpu" > ${log_path}/$2.log 2>&1
    print_info $? $2
else
