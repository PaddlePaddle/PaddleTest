...
class TestPaddleCVPredict():
      ...
      def test_cv_predict(self, run_mode='paddle', device='CPU'):
          ...
          cmd='python -u tools/predict.py --config=%s --input=%s --run_mode=%s --device=%s' % (self.yml, self.input, run_mode, device)
          print(cmd)
      ...
      def test_wheel_predict(self):
          ...
          if os.path.exists('output'):
              shutil.rmtree('output')
          cmd='python -c "from paddlecv import PaddleCV; paddlecv = PaddleCV(task_name=%s); res = paddlecv(%s)"' % (self.model, self.input)
          print(cmd)
