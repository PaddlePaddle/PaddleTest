...
class TestQuantAwareCase1(StaticCase):
    ...
    def get_op_number(self, prog):
        ...
        for op in graph.all_op_nodes():
            # if op.name() in ['conv2d', 'depthwise_conv2d', 'mul', 'conv2d_transpose']:
    ...
    def test_quant_op(self):
        # def quant_op(self):
        """
        paddleslim.quant.quant_aware(program,
        place, config, scope=None,
        for_test=False,
        ...
        quant_prog_1 = quant_aware(train_prog, place, config=config_1, for_test=True)
        op_nums_1, quant_op_nums_1 = self.get_op_number(quant_prog_1)
        convert_prog_1 = convert(quant_prog_1, place, config=config_1)
        convert_op_nums_1, convert_quant_op_nums_1 = self.get_op_number(convert_prog_1)

        config_1["not_quant_pattern"] = ["last_fc"]
        quant_prog_2 = quant_aware(train_prog, place, config=config_1, for_test=True)
        op_nums_2, quant_op_nums_2 = self.get_op_number(quant_prog_2)
        convert_prog_2 = convert(quant_prog_2, place, config=config_1)
        convert_op_nums_2, convert_quant_op_nums_2 = self.get_op_number(convert_prog_2)
    ...
    def te_quant_op1(self):
        ...
        config_1 = {
            "weight_quantize_type": "channel_wise_abs_max",
            "activation_quantize_type": "moving_average_abs_max",
            "quantize_op_types": ["depthwise_conv2d", "mul", "conv2d", "conv2d_transpose"],
        }
        ...
        config_list = [config_0, config_1, config_2, config_3, config_4]
        for config in config_list:
            print("-----config----", config)
            quant_prog_1 = quant_aware(train_prog, place, config=config, for_test=True)
            op_nums_1, quant_op_nums_1 = self.get_op_number_2(quant_prog_1)
            convert_prog_1 = convert(quant_prog_1, place, config=config)
            convert_op_nums_1, convert_quant_op_nums_1 = self.get_op_number_2(convert_prog_1)

            config["not_quant_pattern"] = ["last_fc"]
            quant_prog_2 = quant_aware(train_prog, place, config=config, for_test=True)
            op_nums_2, quant_op_nums_2 = self.get_op_number_2(quant_prog_2)
            convert_prog_2 = convert(quant_prog_2, place, config=config)
            convert_op_nums_2, convert_quant_op_nums_2 = self.get_op_number_2(convert_prog_2)
            ...
            # ----op_nums_1 - --- 28 - -quant_op_nums_1 - -- 60  convert_op_nums_1: 28  convert_quant_op_nums_1: 30
            # ----op_nums_2 - --- 28 - -quant_op_nums_2 - -- 56
            # convert_op_nums_2: 28
            # convert_quant_op_nums_2: 28
...
class TestQuantAwareCase2(StaticCase):
    ...
    def te_accuracy(self):
        ...
        quant_eval_prog, int8_prog = convert(quant_eval_prog, place, config, save_int8=True)
