...
easydata --model image_orientation --input ./demo/image_orientation/1.jpg --device cpu #>../tests/test_PPDC1.log
if [ $? -ne 0 ];
then
    bug=`expr ${bug} + 1`;
fi

easydata --model image_orientation --input ./demo/image_orientation/ --device cpu #>../tests/test_PPDC2.log
if [ $? -ne 0 ];
then
    bug=`expr ${bug} + 1`;
fi

easydata --model image_orientation --input ./demo/image_orientation/ --device cpu --thresh 0.93 #>../tests/test_PPDC3.log
if [ $? -ne 0 ];
then
    bug=`expr ${bug} + 1`;
fi

easydata --model clarity_assessment --input ./demo/clarity_assessment/ --device cpu #>../tests/test_PPDC4.log
if [ $? -ne 0 ];
then
    bug=`expr ${bug} + 1`;
fi
