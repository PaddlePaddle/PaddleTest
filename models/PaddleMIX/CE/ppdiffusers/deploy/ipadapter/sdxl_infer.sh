...
(python infer.py \
    ...
    --backend paddle \
    --device gpu \
    --task_name inpaint_legacy) 2>&1 | tee ${log_dir}/ipadapter_sdxl_inference_inpaint.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
...
(python infer.py \
    ...
    --backend paddle_tensorrt \
    --device gpu \
    --task_name inpaint_legacy) 2>&1 | tee ${log_dir}/ipadapter_sdxl_inference_tensorrt_inpaint.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
