...
line=ppcls/configs/ImageNet/SwinTransformer/SwinTransformer_large_patch4_window12_384.yaml
filename=${line##*/}
#echo $filename
model=${filename%.*}
echo $model
...
python  tools/train.py -c $line  \
    ...
    -o DataLoader.Train.sampler.batch_size=4  > log/${model}_1card.log 2>&1
cat log/${model}_1card.log | grep Train | grep Avg | grep 'Epoch 5/5' > ../log/${model}_1card.log
