...
python tools/train.py -c ppcls/configs/ImageNet/MobileNetV2/MobileNetV2.yaml -o Global.epochs=2 -o DataLoader.Train.sampler.shuffle=False -o DataLoader.Train.sampler.batch_size=4 -o DataLoader.Eval.sampler.batch_size=4 > log/MobileNetV2_1card.log 2>&1
cat log/MobileNetV2_1card.log | grep Train | grep Avg | grep 'Epoch 2/2' > ../log/MobileNetV2_1card.log
