...
class RepoInit():
      def __init__(self, repo):
         ...
         pid = os.getpid()
         cmd='''ps aux| grep python | grep -v %s | awk '{print $2}'| xargs kill -9; rm -rf %s; git clone https://gitee.com/paddlepaddle/%s.git; cd %s; python -m pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple''' % (pid, self.repo, self.repo, self.repo)
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
...
class RepoInitSummer():
      def __init__(self, repo):
         ...
         print("This is Repo Init!")
         pid = os.getpid() 
         cmd='''ps aux | grep python | grep -v %s | awk '{print $2}'| xargs kill -9; rm -rf %s; git clone https://gitee.com/summer243/%s.git; cd %s; python -m pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple''' % (pid, self.repo, self.repo, self.repo)
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
         output=repo_result[1]
         assert exit_code == 0, "git clone %s failed!   log information:%s" % (self.repo, output)
...
class RepoInitCustom():
      def __init__(self, repo):
         ...
         print("This is Repo Init!")
         pid = os.getpid()
         cmd='''ps aux | grep python | grep -v %s | awk '{print $2}'| xargs kill -9; rm -rf %s; git clone https://gitee.com/paddlepaddle/%s.git;'''% (pid, self.repo, self.repo)
         repo_result=subprocess.getstatusoutput(cmd)
         exit_code=repo_result[0]
         output=repo_result[1]
         assert exit_code == 0, "git clone %s failed!   log information:%s" % (self.repo, output)
...
class TestOcrModel():
      ...
      def test_ocr_train(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; sed -i s!data_lmdb_release/training!data_lmdb_release/validation!g %s; python -m paddle.distributed.launch --gpus=0,1,2,3 --log_dir=log_%s  tools/train.py -c %s -o Global.use_gpu=True Global.epoch_num=1 Global.save_epoch_step=1 Global.eval_batch_step=200 Global.print_batch_step=10 Global.save_model_dir=output/%s Train.loader.batch_size_per_card=10 Global.print_batch_step=1' % (self.yaml,  self.model, self.yaml, self.model)
          print(cmd)
      ...
      def test_ocr_eval(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3;  python tools/eval.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest' % (self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_rec_infer(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; python tools/infer_rec.py -c %s  -o Global.use_gpu=True Global.checkpoints=output/%s/latest Global.infer_img=doc/imgs_words/en/word_1.png' % (self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_export_model(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; python tools/export_model.py -c %s -o Global.use_gpu=True Global.checkpoints=output/%s/latest  Global.save_inference_dir=./models_inference/%s' % (self.yaml, self.model, self.model)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_ocr_rec_predict(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; python tools/infer/predict_rec.py --image_dir="./doc/imgs_words_en/word_336.png" --rec_model_dir="./models_inference/"%s --rec_image_shape="3, 32, 100" --rec_char_type="en" --rec_algorithm=CRNN' % (self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          exit_check_fucntion(exit_code, output, 'predict')
      ...
      def test_ocr_det_infer(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; python tools/infer_det.py -c %s -o Global.use_gpu=True Global.checkpoints=output/%s/latest Global.infer_img="./doc/imgs_en/" Global.test_batch_size_per_card=1' % (self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          exit_check_fucntion(exit_code, output, 'infer')
      ...
      def test_ocr_det_predict(self):
          cmd='cd PaddleOCR; export HIP_VISIBLE_DEVICES=0,1,2,3; python tools/infer/predict_det.py --image_dir="./doc/imgs_en/img_10.jpg" --det_model_dir="./models_inference/"%s --det_algorithm=DB ' % (self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          exit_check_fucntion(exit_code, output, 'predict')
...
class TestDetectionDygraphModel():
      ...
      def test_detection_train(self):
          cmd='cd PaddleDetection; export HIP_VISIBLE_DEVICES=0,1,2,3; python -m paddle.distributed.launch --gpus=0,1,2,3 --log_dir=log_%s tools/train.py -c %s -o TrainReader.batch_size=1 epoch=3' % (self.model, self.yaml)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
...
class TestDetectionStaticModel():
      ...
      def test_detection_train(self):
          cmd='cd PaddleDetection/static; export HIP_VISIBLE_DEVICES=0,1,2,3; python -m paddle.distributed.launch --gpus=0,1,2,3 --log_dir=log_%s tools/train.py -c %s -o TrainReader.batch_size=1  -o max_iters=10' % (self.model, self.yaml)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
...
class TestSegModel():
      ...
      def test_seg_train(self):
          cmd='cd PaddleSeg; sed -i s/"iters: 80000"/"iters: 50"/g %s; rm -rf log; export HIP_VISIBLE_DEVICES=0,1,2,3; python -u -m paddle.distributed.launch --gpus="0,1,2,3" --log_dir=log_%s train.py --config %s --do_eval --use_vdl --num_workers 6 --save_dir log/%s --save_interval 50 --iters 50' % (self.yaml, self.model, self.yaml, self.model)
          print(cmd)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
      ...
      def test_seg_eval(self):
          cmd='cd PaddleSeg; wget https://bj.bcebos.com/paddleseg/dygraph/cityscapes/%s/model.pdparams; mv model.pdparams %s.pdparams; export HIP_VISIBLE_DEVICES=0,1,2,3; python -m paddle.distributed.launch val.py --config %s --model_path=%s.pdparams' % (self.model, self.model, self.yaml, self.model)
          detection_result = subprocess.getstatusoutput(cmd)
          exit_code = detection_result[0]
          output = detection_result[1]
          exit_check_fucntion(exit_code, output, 'eval')
...
class TestGanModel():
      ...
      def test_gan_train(self):
          cmd='cd PaddleGAN; sed -i 1s/epochs/total_iters/ %s; export HIP_VISIBLE_DEVICES=0,1,2,3; python -u -m paddle.distributed.launch --gpus="0,1,2,3" tools/main.py --config-file %s -o total_iters=20 snapshot_config.interval=10 log_config.interval=1 output_dir=output' % (self.yaml, self.yaml)
          print(cmd)
