...
def setup_module():
    ...
    RepoDataset(cmd='''cd PaddleVideo; 
                       cd data; rm -rf ucf101; ln -s /data/ucf101 ucf101;
                       rm -rf TSM_k400.pdparams; wget https://videotag.bj.bcebos.com/PaddleVideo-release2.1/TSM/TSM_k400.pdparams; 
                       mkdir dataset
                       cd dataset
    ...
    RepoDataset(cmd='''cd models/PaddleCV/video;
                      cd data;
...
def test_slowfast():
    ...
    cmd = '''cd PaddleVideo; python -B -m paddle.distributed.launch --gpus="0,1,2,3" --log_dir=log_slowfast_test main.py --test -c  configs/recognition/slowfast/slowfast.yaml -w data/SlowFast.pdparams -o DATASET.test.file_path=data/k400/val_small.list''' 
    model.test_video_eval(cmd=cmd)

def test_tsn():
    model = TestVideoModel(model='tsn', yaml='configs/recognition/tsn/tsn_k400_frames.yaml')
    cmd='''cd PaddleVideo; python3.7 -B -m paddle.distributed.launch --gpus="0,1,2,3" --log_dir=log_tsn main.py  --test -c configs/recognition/tsn/tsn_k400_frames.yaml -w "data/TSN_k400.pdparams" -o DATASET.test.format=VideoDataset -o DATASET.test.file_path=data/k400/val_small.list -o DATASET.test.suffix="" -o PIPELINE.test.decode.name=VideoDecoder'''
    model.test_video_eval(cmd=cmd)

def test_attention_lstm():
    model = TestVideoModel(model='attention_lstm', yaml='./configs/attention_lstm.yaml')
    cmd='''cd models/PaddleCV/video;  sed -i s/test.list/test_small.list/g configs/attention_lstm.yaml; python eval.py --model_name=AttentionLSTM --config=./configs/attention_lstm.yaml --log_interval=1 --weights=data/AttentionLSTM.pdparams --use_gpu=True'''
    model.test_video_eval(cmd=cmd)


def test_stnet():
    model = TestVideoModel(model='stnet', yaml='./configs/stnet.yaml')
    cmd='''cd models/PaddleCV/video; sed -i s/pkl/mp4/g configs/stnet.yaml; sed -i s/test.list/test_small.list/g ./configs/stnet.yaml; python eval.py --model_name=STNET --config=./configs/stnet.yaml --log_interval=1 --weights=data/STNET.pdparams --use_gpu=True'''
    model.test_video_eval(cmd=cmd)
