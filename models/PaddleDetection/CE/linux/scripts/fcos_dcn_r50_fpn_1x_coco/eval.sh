...
eval_model_single(){
    python tools/eval.py \
       -c configs/fcos/fcos_dcn_r50_fpn_1x_coco.yml \
       -o weights=https://paddledet.bj.bcebos.com/models/fcos_dcn_r50_fpn_1x_coco.pdparams >../log/${model}/${model}_eval_single.log
    if [ $? -ne 0 ];then
...
eval_model_multi(){
    python -m paddle.distributed.launch \
       tools/eval.py \
       -c configs/fcos/fcos_dcn_r50_fpn_1x_coco.yml \
       -o weights=https://paddledet.bj.bcebos.com/models/fcos_dcn_r50_fpn_1x_coco.pdparams >../log/${model}/${model}_eval_multi.log
    if [ $? -ne 0 ];then
