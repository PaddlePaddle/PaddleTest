...
eval_model_single(){
    python tools/eval.py \
       ...
       -o weights=https://paddledet.bj.bcebos.com/models/yolov3_darknet53_270e_coco.pdparams >../log/${model}/${model}_eval_single.log
    if [ $? -ne 0 ];then
...
eval_model_multi(){
    python -m paddle.distributed.launch \
       tools/eval.py \
       -c configs/yolov3/yolov3_darknet53_270e_coco.yml \
       -o weights=https://paddledet.bj.bcebos.com/models/yolov3_darknet53_270e_coco.pdparams >../log/${model}/${model}_eval_multi.log
    if [ $? -ne 0 ];then
