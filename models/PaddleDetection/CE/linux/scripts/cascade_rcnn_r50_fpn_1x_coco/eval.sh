...
eval_model_single(){
    python tools/eval.py \
       ...
       -o weights=https://paddledet.bj.bcebos.com/models/cascade_rcnn_r50_fpn_1x_coco.pdparams >../log/${model}/${model}_eval_single.log
    if [ $? -ne 0 ];then
...
eval_model_multi(){
    python -m paddle.distributed.launch \
       tools/eval.py \
       -c configs/cascade_rcnn/cascade_rcnn_r50_fpn_1x_coco.yml \
       -o weights=https://paddledet.bj.bcebos.com/models/cascade_rcnn_r50_fpn_1x_coco.pdparams >../log/${model}/${model}_eval_multi.log
    if [ $? -ne 0 ];then
