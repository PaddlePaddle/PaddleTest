...
train_model_multi(){
    ...
    tools/train.py \
           ...
           --enable_ce True \
           -o TrainReader.shuffle=false TrainReader.batch_size=12 epoch=1 worker_num=0 \
           LearningRate.base_lr=0.00125 >../log/${model}/${model}_train_multi.log 2>&1
    print_info $? multi
}
train_model_single(){
    python tools/train.py \
       ...
       --enable_ce True \
       -o TrainReader.shuffle=false TrainReader.batch_size=12 epoch=1 worker_num=0 \
       LearningRate.base_lr=0.000625  >../log/${model}/${model}_train_single.log 2>&1
    print_info $? single
}
