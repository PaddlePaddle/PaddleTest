...
train_model_multi(){
    ...
    tools/train.py \
           -c configs/ttfnet/ttfnet_darknet53_1x_coco.yml \
           --enable_ce True \
           -o TrainReader.shuffle=false epoch=1 worker_num=0 >../log/${model}/${model}_train_multi.log 2>&1
    print_info $? multi
}
train_model_single(){
    python tools/train.py \
           -c configs/ttfnet/ttfnet_darknet53_1x_coco.yml \
           --enable_ce True \
           -o TrainReader.shuffle=false epoch=1 worker_num=0 >../log/${model}/${model}_train_single.log 2>&1
    print_info $? single
}
