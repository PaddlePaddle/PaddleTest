...
train_model_multi(){
    ...
    tools/train.py \
           -c configs/keypoint/tiny_pose/tinypose_128x96.yml \
           --enable_ce True \
           -o TrainReader.shuffle=false epoch=1 worker_num=0 \
           LearningRate.base_lr=0.008 >../log/${model}/${model}_train_multi.log 2>&1
    print_info $? multi
}
train_model_single(){
    python tools/train.py \
           ...
           --enable_ce True \
           -o TrainReader.shuffle=false epoch=1 worker_num=0 \
           LearningRate.base_lr=0.004 >../log/${model}/${model}_train_single.log 2>&1
    print_info $? single
}
