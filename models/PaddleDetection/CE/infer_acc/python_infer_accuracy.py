                    ...
                    if model_type == 'det':
                        ...
                        for line in boxes_line:
                            ...
                            boxcoordinates_list.append(
                                'box:{},{},{},{}'.format(box_coordinates[0], box_coordinates[1], box_coordinates[2],
                                                         box_coordinates[3]))
                return classid_list, confidence_list, boxcoordinates_list
        except:
            ...
            if type_list == 'class_id':
                for i in range(num_sample):
                    if float(run_list[i].split(':')[1]) != float(sample_list[i].split(':')[1]):
                        ...
                        print('{} {}: RUN {} ，SAMPLE  {} ，CHECK PLEASE !'.format(model_name, type_list, run_list[i],
                                                                                 sample_list[i]))
                ...
                for i in range(num_sample):
                    ...
                    for inx, j in enumerate(run_box):
                        if j > sample_box[inx] * (1 + box_thr) or j < sample_box[inx] * (1 - box_thr):
                            type_list_sign = 'false'
                            print(
                                '{} {}: RUN {} ，SAMPLE  {} ，CHECK PLEASE !'.format(model_name, type_list, run_list[i],
                                                                                   sample_list[i]))
...
def run_pip(model_sample, model_run, model_type, model_name):
    ...
    if model_type == 'keypoint':
        rectangular_sample, confidence_sample, kpcoordinates_sample = get_main_json_data(model_sample,
        ...
        classid_sample, confidence_sample, boxcoordinates_sample = get_main_txt_data(model_sample, model_type,
                                                                                     model_name=model_name)
        classid_run, confidence_run, boxcoordinates_run = get_main_txt_data(model_run, model_type,
                                                                            model_name=model_name)
