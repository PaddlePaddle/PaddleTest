...
from ppdet.modeling.heads import OTAVFLHead
...
class Config:
    def __init__(self):
        paddle.seed(33)
        self.module_name = "OTAVFLHead"
        distributionfocalloss = DistributionFocalLoss()
        varifocalloss = VarifocalLoss()
        fcosfeat = FCOSFeat(feat_in=8, feat_out=8)
        self.net = OTAVFLHead(conv_feat=fcosfeat, dgqp_module=None, num_classes=3, fpn_stride=[8, 16, 32], prior_prob=0.01, loss_class=varifocalloss, loss_dfl=distributionfocalloss, loss_bbox='GIoULoss', assigner='SimOTAAssigner', reg_max=16, feat_in_chan=8, nms=None, nms_pre=100, cell_offset=0)
        self.net.eval()
