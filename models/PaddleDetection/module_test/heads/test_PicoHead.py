...
from ppdet.modeling.heads import PicoHead
...
class Config:
    def __init__(self):
        paddle.seed(33)
        self.module_name = "PicoHead"
        varifocalloss = VarifocalLoss()
        picofeat = PicoFeat(feat_in=8, feat_out=8)
        self.net = PicoHead(conv_feat=picofeat, dgqp_module=None, num_classes=3, fpn_stride=[8 ,16 ,32], prior_prob=0.01, loss_class=varifocalloss, loss_dfl='DistributionFocalLoss', loss_bbox='GIoULoss', assigner='SimOTAAssigner', reg_max=16, feat_in_chan=8, nms=None, nms_pre=100)
        self.net.eval()
