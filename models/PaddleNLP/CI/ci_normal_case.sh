...
for exec_file in `ls`;do
    # Data Prepare
    if [[ ${exec_file} == "requirements" ]];then
    ...
    # TRAIN
        ...
        python -m paddle.distributed.launch ${exec_file} --save_steps ${save_steps} --max_steps ${max_steps} --output_dir ${output_dir} >${log_path}/${example}_train>>${log_path}/${example}_train 2>&1
