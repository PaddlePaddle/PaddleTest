...
def run_normal_case(case_path):
    ...
    for train_file in model_info["train_exec_file"]:
        train_output = subprocess.getstatusoutput("python -m paddle.distributed.launch %s --devices gpu --max_steps 2 \
            --save_steps 2 --output_dir ./output/" % (train_file))
    ...
    if export_exec_file:
        export_output = subprocess.getstatusoutput("python %s --export_output_dir ./inference_model/" % \
            (export_exec_file))
        save_log(export_output[0], export_output[1], export_exec_file.split(".")[0])
    else:
        print("export model skipped !")
    if infer_exec_file:
        infer_output = subprocess.getstatusoutput("cd %s && python %s --inference_model_dir ../../inference_model/" % \
            (depoly_path, infer_exec_file))
