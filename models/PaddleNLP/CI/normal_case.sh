...
export infer_output=$model_dir/infer_output
export infer_input=$model_dir/infer_input
print_info(){
if [ $1 -ne 0 ];then
...
for exec_file in `ls`;do
    ...
    # EVAL
        ...
        python ${exec_file} --device ${devices}  --checkpoint ${output}  >${log_path}/${example}_eval >>${log_path}/${example}_eval 2>&1
        print_info $? ${example}_eval
    # PREDICT
    elif [[ ${exec_file} == "predict.py" ]] || [[ ${exec_file} =~ "run_predict" ]] ;then
        python ${exec_file} --device ${devices}  --checkpoint ${output}  >${log_path}/${example}_predict >>${log_path}/${example}_predcit 2>&1
        print_info $? ${example}_predict
    # EXPORT MODEL
    elif [[ ${exec_file} == "export_model.py" ]] ;then
        python ${exec_file} --device ${devices}  --checkpoint ${output} --infer_output ${infer_output}  >${log_path}/${example}_export_model >>${log_path}/${example}_export_model 2>&1
        print_info $? ${example}_export_model
    # INFER
    elif [[ ${exec_file} == "infer.py" ]] || [[ ${exec_file} =~ "run_infer" ]] ;then
        python ${exec_file} --device ${devices}  --infer_input ${infer_input}   >${log_path}/${example}_predict >>${log_path}/${example}_predcit 2>&1
        print_info $? ${example}_predict
    # CUSTOM
