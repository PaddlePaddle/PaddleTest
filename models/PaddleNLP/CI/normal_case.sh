...
for exec_file in `ls`;do
    ...
    # TRAIN
        ...
        python -m paddle.distributed.launch ${exec_file} --device ${devices} --max_steps ${max_steps} --save_steps ${save_steps}  >${log_path}/${example}_train>>${log_path}/${example}_train 2>&1
        print_info $? ${example}_train
    # EVAL
    elif [[ ${exec_file} == "eval.py" ]] || [[ ${exec_file} =~ "run_eval" ]] ;then
        python ${exec_file} --device ${devices}    >${log_path}/${example}_eval >>${log_path}/${example}_eval 2>&1
        print_info $? ${example}_eval
    # PREDICT
    elif [[ ${exec_file} == "predict.py" ]] || [[ ${exec_file} =~ "run_predict" ]] ;then
        python ${exec_file} --device ${devices}    >${log_path}/${example}_predict >>${log_path}/${example}_predcit 2>&1
        print_info $? ${example}_predict
    # EXPORT MODEL
    elif [[ ${exec_file} == "export_model.py" ]] ;then
        python ${exec_file} --device ${devices}   >${log_path}/${example}_export_model >>${log_path}/${example}_export_model 2>&1
