...
if __name__ == "__main__":
    pwd = os.getcwd()
    wav = os.path.join(args.audio_path, "LJ001-0003.wav")  # 存储在本地的需要预测的wav文件
    sr = 44100  # 音频文件的采样率
    ...
    result = model.predict(data, sample_rate=sr, batch_size=1, feat_type="mel", use_gpu=args.use_gpu)
    print(result[0])  # result[0]包含音频文件属于各类别的概率值
    print("***" * 20)
    topk = 10  # 展示音频得分前10的标签和分数
    # 读取audioset数据集的label文件
    label_file = os.path.join(MODULE_HOME, args.model_name, "audioset_labels.txt")
    label_map = {}
