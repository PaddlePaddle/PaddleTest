...
# train
rm -rf runs
python train.py --data=ljspeech_waveflow/ --output=runs/test --device="gpu" --nprocs=1 --opts data.batch_size 2 training.max_iteration 500 training.valid_interval 500 training.save_interval 500 > ../../log/waveflow_1card.log 2>&1
cat ../../log/waveflow_1card.log | grep "step: 500" | awk 'BEGIN{FS=","} {print $4}' > ./tmp_1card.log
