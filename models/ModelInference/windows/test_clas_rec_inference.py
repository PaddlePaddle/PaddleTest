...
def setup_module():
    ...
    RepoDataset(cmd='''set https_proxy=& set http_proxy=& cd PaddleClas/deploy &  md models & cd models & wget --no-check-certificate https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/rec/models/inference/ppyolov2_r50vd_dcn_mainbody_v1.0_infer.tar & tar xf ppyolov2_r50vd_dcn_mainbody_v1.0_infer.tar & del /q /f *.tar & cd .. & wget --no-check-certificate https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/rec/data/recognition_demo_data_v1.1.tar & tar xf recognition_demo_data_v1.1.tar & del /q /f *.tar''')
