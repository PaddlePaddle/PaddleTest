...
class RepoInit:
    ...
    def __init__(self, repo, branch):
        ...
        cmd = """rd /s /q %s & git clone https://github.com/paddlepaddle/%s.git & cd %s & \
...
class TestClasInference:
    ...
    def test_get_pretrained_model(self):
        ...
        if self.model in legendary_models:
            cmd = (
                "set https_proxy=& set http_proxy=& cd PaddleClas & \
...
https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/legendary_models/%s_pretrained.pdparams"
            ...
            cmd = (
                "set https_proxy=& set http_proxy=& cd PaddleClas & \
wget --no-check-certificate \
https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/%s_pretrained.pdparams"
                % self.model
            )
...
class TestClasRecInference:
    ...
    def test_get_clas_rec_inference_model(self, model):
        """
          ...
          """
        cmd = (
            "set https_proxy=& set http_proxy=& cd PaddleClas/deploy & cd models \
& wget --no-check-certificate https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/rec/models/inference/%s.tar \
& tar xf %s.tar & rm -rf %s.tar & cd .."
  ...
  -o Global.use_fp16=False -o Global.enable_mkldnn=True""" % (
        ...
        for cmd in [cmd_gpu, cmd_trt, cmd_fp16, cmd_cpu, cmd_mkldnn]:
            ...
            with assume:
                assert rec_scores == approx(expect_rec_scores, abs=2e-3), (
...
class TestOcrRecInference:
    ...
    def test_get_ocr_rec_inference_model(self, category):
        ...
        custom_models = [
            ...
            "ka_mobile_v2.0_rec_infer",
            "ta_mobile_v2.0_rec_infer",
            "te_mobile_v2.0_rec_infer",
            "german_mobile_v2.0_rec_infer",
        ]
        if self.model in custom_models:
            cmd = """cd PaddleOCR & wget --no-check-certificate -P %s https://paddleocr.bj.bcebos.com/%s/%s.tar \
& cd %s & tar xf %s.tar &
                    rm -rf %s.tar & cd ..""" % (
                self.model,
                category,
            ...
            cmd = """cd PaddleOCR & wget --no-check-certificate https://paddleocr.bj.bcebos.com/%s/%s.tar \
& tar xf %s.tar &
                    rm -rf %s.tar""" % (
                category,
                self.model,
    ...
    def test_ocr_rec_predict(self, expect_rec_docs, expect_rec_scores):
        """
          ...
          """
        cmd_gpu = (
            'cd PaddleOCR & python tools/infer/predict_rec.py --image_dir=%s --rec_model_dir=%s \
--rec_image_shape="3, 32, 100" --rec_char_dict_path=%s --rec_algorithm=CRNN --use_gpu=True --use_tensorrt=False\
 --precision=fp32 --enable_mkldnn=False --rec_batch_num=1'
            % (self.infer_imgs, self.model, self.rec_char_dict)
        )
        cmd_trt = (
            'cd PaddleOCR & python tools/infer/predict_rec.py --image_dir=%s --rec_model_dir=%s \
--rec_image_shape="3, 32, 100" --rec_char_dict_path=%s --rec_algorithm=CRNN --use_gpu=True --use_tensorrt=True\
 --precision=fp32 --enable_mkldnn=False --rec_batch_num=1'
            % (self.infer_imgs, self.model, self.rec_char_dict)
        )
        cmd_cpu = (
            'cd PaddleOCR & python tools/infer/predict_rec.py --image_dir=%s --rec_model_dir=%s \
--rec_image_shape="3, 32, 100" --rec_char_dict_path=%s --rec_algorithm=CRNN --use_gpu=False --use_tensorrt=False\
 --precision=fp32 --enable_mkldnn=False --rec_batch_num=1'
            % (self.infer_imgs, self.model, self.rec_char_dict)
        )
        cmd_mkldnn = (
            'cd PaddleOCR & python tools/infer/predict_rec.py --image_dir=%s --rec_model_dir=%s \
--rec_image_shape="3, 32, 100"  --rec_char_dict_path=%s --rec_algorithm=CRNN --use_gpu=False --use_tensorrt=False\
 --precision=fp32 --enable_mkldnn=True --rec_batch_num=1'
            % (self.infer_imgs, self.model, self.rec_char_dict)
        )
...
class TestOcrDetInference:
    ...
    def test_get_ocr_det_inference_model(self, category):
        """
          ...
          """
        cmd = """cd PaddleOCR & wget --no-check-certificate https://paddleocr.bj.bcebos.com/%s/%s_infer.tar \
& tar xf %s_infer.tar & rm -rf %s_infer.tar""" % (
            category,
            self.model,
            self.model,
            self.model,
    ...
    def test_get_ocr_det_train_model(self, category):
        """
          get ocr_det inference mode
          """
        cmd = """cd PaddleOCR & wget --no-check-certificate https://paddleocr.bj.bcebos.com/%s/%s_train.tar
& tar xf %s_train.tar & rm -rf %s_train.tar""" % (
            category,
            self.model,
            self.model,
            self.model,
