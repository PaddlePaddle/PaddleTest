...
ImageNet|slim|metric_learning)
    if [[ ${1} =~ 'ultra' ]];then
        python python/predict_cls.py -c configs/inference_cls_ch4.yaml  \
            ...
            > ../${log_path}/predict/${model_name}_${output_type}.log 2>&1
    else
...
;;
reid)
    echo "predict_exit_code: unspported" > ../${log_path}/predict/${model_name}_${output_type}.log
;;
Vehicle)
...
if [[ $? -eq 0 ]] && [[ $(grep -c  "Error" ../${log_path}/predict/${model_name}_${output_type}.log) -eq 0 ]];then
    echo -e "\033[33m predict of ${model_name}_${output_type}  successfully!\033[0m"| tee -a ../${log_path}/result.log
    echo "predict_exit_code: 0.0" >> ../${log_path}/predict/${model_name}_${output_type}.log
else
    cat ../${log_path}/predict/${model_name}_${output_type}.log
    echo -e "\033[31m predict of ${model_name}_${output_type} failed!\033[0m"| tee -a ../${log_path}/result.log
    echo "predict_exit_code: 1.0" >> ../${log_path}/predict/${model_name}_${output_type}.log
fi
