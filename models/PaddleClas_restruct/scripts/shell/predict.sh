...
export predict_type=${3:-pretrained}
...
# for predict_type in ${arr[@]}
# do
\cp -r -f ${Project_path}/../scripts/shell/choose_model.sh .
export predict_step=True
source choose_model.sh

size_tmp=`cat ${yaml_line} |grep image_shape|cut -d "," -f2|cut -d " " -f2` #获取train的shape保持和predict一致
cd deploy
...
ImageNet|slim|metric_learning)
    if [[ ${yaml_line} =~ 'ultra' ]];then
        python python/predict_cls.py -c configs/inference_cls_ch4.yaml  \
            ...
            > ../${log_path}/predict/${model_name}_${predict_type}.log 2>&1
    else
...
;;
reid)
    echo "predict_exit_code: unspported" > ../${log_path}/predict/${model_name}_${predict_type}.log
;;
Vehicle)
...
# if [[ $? -eq 0 ]] && [[ $(grep -c  "Error" ../${log_path}/predict/${model_name}_${predict_type}.log) -eq 0 ]];then
if [[ $? -eq 0 ]];then
    echo -e "\033[33m predict of ${model_name}_${predict_type}  successfully!\033[0m"| tee -a ../${log_path}/result.log
    echo "predict_exit_code: 0.0" >> ../${log_path}/predict/${model_name}_${predict_type}.log
else
    cat ../${log_path}/predict/${model_name}_${predict_type}.log
    echo -e "\033[31m predict of ${model_name}_${predict_type} failed!\033[0m"| tee -a ../${log_path}/result.log
    echo "predict_exit_code: 1.0" >> ../${log_path}/predict/${model_name}_${predict_type}.log
fi
