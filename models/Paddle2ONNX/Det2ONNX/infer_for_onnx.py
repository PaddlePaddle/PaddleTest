...
class Detector(object):
    ...
    def predict(self, image_list, threshold=0.5, repeats=1, add_timer=True, img_idx=0):
        """
        Args:
        ...
        for i, value in enumerate(input_names):
            ...
            # print('input_tensor is: ', input_tensor.numpy())
        ...
        if reduce(lambda x, y: x * y, np_boxes.shape) < 6:
            ...
            results = self.postprocess(np_boxes, np_masks, inputs, np_boxes_num, threshold=threshold)
        if add_timer:
            self.det_times.postprocess_time_s.end()
            self.det_times.img_num += len(image_list)
        print("results is: ", results)
        np.save(os.path.join(FLAGS.infer_output_np, "output_" + str(img_idx)), results["boxes"])
        return results
...
class DetectorSOLOv2(Detector):
    ...
    def predict(self, image, threshold=0.5, repeats=1, add_timer=True):
        ...
        return dict(segm=np_segms, label=np_label, score=np_score, boxes_num=np_boxes_num)
...
def visualize(image_list, results, labels, output_dir="output/", threshold=0.5):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        if "label" in results:
            im_results["label"] = results["label"][start_idx : start_idx + im_bboxes_num]
        if "score" in results:
            im_results["score"] = results["score"][start_idx : start_idx + im_bboxes_num]
...
def predict_image(detector, image_list, batch_size=1):
    ...
    for i in range(batch_loop_cnt):
        ...
        if FLAGS.run_benchmark:
            # warmup
            detector.predict(batch_image_list, FLAGS.threshold, repeats=10, add_timer=False)
            # run benchmark
...
def predict_video(detector, camera_id):
    ...
    while 1:
        ...
        im = visualize_box_mask(frame, results, detector.pred_config.labels, threshold=FLAGS.threshold)
        im = np.array(im)
        writer.write(im)
        if camera_id != -1:
