...
class KeyPoint_Detector(Detector):
    ...
    def predict(self, image_list, threshold=0.5, repeats=1, add_timer=True):
        ...
        # postprocess
        results = self.postprocess(np_boxes, np_masks, inputs, threshold=threshold)
        if add_timer:
...
def predict_image(detector, image_list):
    ...
    for i, img_file in enumerate(image_list):
        if FLAGS.run_benchmark:
            ...
            draw_pose(img_file, results, visual_thread=FLAGS.threshold, save_dir=FLAGS.output_dir)
...
def predict_video(detector, camera_id):
    ...
    while 1:
        ...
        im = draw_pose(frame, results, visual_thread=FLAGS.threshold, returnimg=True)
        writer.write(im)
        if camera_id != -1:
