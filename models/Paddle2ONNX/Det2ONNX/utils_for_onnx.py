...
def argsparser():
    ...
    parser.add_argument(
        "--input_np", type=str, default="input_np", help="Directory of input np.array for onnx acc test."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_dark", type=bool, default=True, help="whether to use darkpose to get better keypoint position predict "
    )
    return parser
...
class Timer(Times):
    ...
    def info(self, average=False):
        ...
        default
        """
        total_time = self.preprocess_time_s.value() + self.inference_time_s.value() + self.postprocess_time_s.value()
        total_time = round(total_time, 4)
        print("------------------ Inference Time Info ----------------------")
        print("total_time(ms): {}, img_num: {}".format(total_time * 1000, self.img_num))
        preprocess_time = (
            round(self.preprocess_time_s.value() / max(1, self.img_num), 4)
            if average
            else self.preprocess_time_s.value()
        )
        postprocess_time = (
            round(self.postprocess_time_s.value() / max(1, self.img_num), 4)
            if average
            else self.postprocess_time_s.value()
        )
        inference_time = (
            round(self.inference_time_s.value() / max(1, self.img_num), 4) if average else self.inference_time_s.value()
        )
    ...
    def report(self, average=False):
        ...
        """
        dic = {}
        dic["preprocess_time_s"] = (
            round(self.preprocess_time_s.value() / max(1, self.img_num), 4)
            if average
            else self.preprocess_time_s.value()
        )
        dic["postprocess_time_s"] = (
            round(self.postprocess_time_s.value() / max(1, self.img_num), 4)
            if average
            else self.postprocess_time_s.value()
        )
        dic["inference_time_s"] = (
            round(self.inference_time_s.value() / max(1, self.img_num), 4) if average else self.inference_time_s.value()
        )
