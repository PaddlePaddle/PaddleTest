...
class SegV2Test(object):
    """
    test Seg to onnx tipc
    """

    def __init__(self):
        if os.path.exists("tipc_models_url_PaddleSeg_latest.txt"):
            os.remove("tipc_models_url_PaddleSeg_latest.txt")

        self.txt_url = (
            "https://paddle-qa.bj.bcebos.com/fullchain_ce_test/"
            "model_download_link/tipc_models_url_PaddleSeg_latest.txt"
        )

        os.system("wget -q --no-proxy {}".format(self.txt_url))

        self.model_url_list = []
        for line in open("tipc_models_url_PaddleSeg_latest.txt"):
