...
class ClsPredictor(Predictor):
    ...
    def __init__(self, config):
        ...
        if "PreProcess" in config:
            if "transform_ops" in config["PreProcess"]:
                self.preprocess_ops = create_operators(config["PreProcess"]["transform_ops"])
    ...
    def predict(self, images, img_idx=0):
        ...
        print("No.{} batch_output after postprocess is: {}".format(img_idx, batch_output))
        # np.save(os.path.join(args.model_name, 'infer_output_np', 'output_postp_' + str(img_idx)), batch_output)
        return batch_output
...
def main(config):
    ...
    for idx, img_path in enumerate(image_list):
        ...
        if img is None:
            logger.warning("Image file failed to read and has been skipped. The path: {}".format(img_path))
        else:
            img = img[:, :, ::-1]
        ...
        if cnt % config["Global"]["batch_size"] == 0 or (idx + 1) == len(image_list):
            ...
            for number, result_dict in enumerate(batch_results):
                ...
                scores_str = "[{}]".format(", ".join("{:.2f}".format(r) for r in result_dict["scores"]))
                label_names = result_dict["label_names"]
                print(
                    "{}:\tclass id(s): {}, score(s): {}, label_name(s): {}".format(
                        filename, clas_ids, scores_str, label_names
                    )
                )
            batch_imgs = []
            batch_names = []
