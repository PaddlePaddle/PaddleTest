...
def print_dict(d, delimiter=0):
    ...
    for k, v in sorted(d.items()):
        if isinstance(v, dict):
            ...
            logger.info(
                "{}{} : {}".format(delimiter * " ", logger.coloring(k, "HEADER"), logger.coloring(v, "OKGREEN"))
            )
        if k.isupper():
...
def parser():
    ...
    parser.add_argument("-v", "--verbose", action="store_true", help="wheather print the config info")
